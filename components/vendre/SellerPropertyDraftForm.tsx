...
const INTENT_COPY: Record<SellerIntent, { eyebrow: string; title: string; submit: string }> = {
  publish: {
    eyebrow: "Publier mon annonce",
    title: "Préparons une annonce claire et rassurante",
    submit: "Enregistrer mon dossier",
  },
  estimate: {
    eyebrow: "Estimer mon bien",
    title: "Décrivons le même bien avant toute estimation",
    submit: "Enregistrer et demander mes repères",
  },
  professional: {
    eyebrow: "Être accompagné",
    title: "Préparons un dossier utile pour le professionnel",
    submit: "Enregistrer et demander un accompagnement",
  },
};
  ...
  try {
    ...
    if (width < 1200 || height < 800) {
      return {
        ...
        width,
        height,
        message: "La photo est un peu petite. Une image d’au moins 1200 × 800 sera plus nette.",
      };
    }
    return {
      id,
      ...
      width,
      height,
      message: "Bonne taille pour présenter votre bien.",
    };
  } catch {
...
export function SellerPropertyDraftForm({
  ...
  useEffect(() => {
    try {
      ...
      if (!saved) return;
      const parsed = JSON.parse(saved) as { form?: Partial<FormState>; intent?: SellerIntent };
      if (parsed.form) setForm((current) => ({ ...current, ...parsed.form, consent: false }));
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ form: { ...form, consent: false }, intent }));
    }, 250);
  ...
  async function submit() {
    ...
    try {
      ...
      setResult({ ok: false, error: "La connexion a été interrompue. Votre brouillon reste enregistré sur cet appareil." });
    } finally {
      setBusy(false);
    }
  }
    ...
    return (
      <div className={`${ui.surface} mx-auto max-w-xl p-8 text-center`}>
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          Rien n’est publié automatiquement. Nous avons enregistré les informations que vous avez déclarées et votre choix de parcours.
        </p>
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start">
      <div>
        <Link href="/vendre" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-primary">{copy.eyebrow}</p>
            <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">{copy.title}</h1>
          </div>
        </div>

        <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">
          Vendre, estimer ou demander de l’aide commence par le même dossier. Vous avancez à votre rythme et votre brouillon reste sur cet appareil.
        </p>
        {restored ? (
          <p className="mt-3 inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
            <Check size={14} aria-hidden="true" /> Votre brouillon a été repris.
          </p>
        ) : null}
          ...
          {["Votre bien", "Détails", "Photos", "Contact"].map((label, index) => {
            ...
            return (
              <button
                ...
                className={`min-h-14 rounded-xl border px-2 py-2 text-[11px] font-extrabold transition ${
                  active ? "border-primary bg-primary text-primary-foreground" : done ? "border-emerald-300 bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300" : "border-border/30 bg-card text-muted-foreground"
                }`}
              >
                <span className="block text-[10px] opacity-75">{done ? "✓" : number}</span>
                {label}
              </button>
            );
          })}
        ...
        <section className={`${ui.surface} mt-4 p-5 sm:p-7`} aria-live="polite">
            ...
            <div>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className={labelClass}>État du bien
                  <select className={inputClass} value={form.condition} onChange={(event) => set("condition", event.target.value)}>
          ...
          {step === 3 ? (
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">Les photos sont vérifiées sur votre appareil. Elles ne sont pas envoyées dans ce LOT et vous pourrez les remplacer avant la publication.</p>
                ...
                {photos.map((photo) => (
                  <div key={photo.id} className={`rounded-xl border p-3 ${photo.accepted ? "border-emerald-200 bg-emerald-50 dark:bg-emerald-950/30" : "border-amber-200 bg-amber-50 dark:bg-amber-950/30"}`}>
                    <div className="flex items-start gap-2">
                      <Camera size={16} className={photo.accepted ? "text-emerald-700" : "text-amber-700"} aria-hidden="true" />
                      <div className="min-w-0">
                        <p className="truncate text-xs font-extrabold">{photo.name}</p>
                        <p className="mt-1 text-xs leading-5 text-muted-foreground">{photo.message}</p>
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-5 rounded-xl bg-primary/8 p-4 text-xs leading-5 text-muted-foreground">
                <strong className="text-foreground">Pour une annonce très complète :</strong> façade ou extérieur, séjour, cuisine, chambres, salle de bain et vue lorsque c’est pertinent.
              </div>
            </div>
          ) : null}
          ...
          {step === 4 ? (
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">Votre numéro reste privé et n’est pas publié automatiquement.</p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl bg-surface-muted p-4 text-sm leading-6">
                <input type="checkbox" checked={form.consent} onChange={(event) => set("consent", event.target.checked)} className="mt-1 h-4 w-4" />
                <span>Je confirme que ces informations décrivent mon bien et j’accepte d’être recontacté. Je comprends que rien n’est publié automatiquement.</span>
              </label>
              {!readiness.essentialsComplete ? (
                <p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-800 dark:bg-amber-950/30 dark:text-amber-300">Ajoutez le type de bien, la ville, la surface et un téléphone pour enregistrer le dossier.</p>
              ) : null}
          ...
          <div className="mt-7 flex items-center justify-between gap-3 border-t border-border/20 pt-5">
            <button type="button" onClick={() => setStep((current) => Math.max(1, current - 1))} disabled={step === 1} className={`${ui.secondaryAction} disabled:invisible`}>
              <ArrowLeft size={15} aria-hidden="true" /> Précédent
            </button>
            {step < 4 ? (
              <button type="button" onClick={() => setStep((current) => Math.min(4, current + 1))} className={ui.primaryAction}>
                Continuer <ArrowRight size={15} aria-hidden="true" />
              </button>
            ) : (
              <button type="button" disabled={!canSubmit} onClick={() => void submit()} className={`${ui.primaryAction} disabled:cursor-not-allowed disabled:opacity-50`}>
                {busy ? "Enregistrement…" : copy.submit}
      ...
      <aside className="lg:sticky lg:top-24" aria-label="Préparation de l’annonce">
        <section className={`${ui.surface} p-5`}>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-primary">Annonce prête</p>
              <p className="mt-1 text-lg font-extrabold">{readiness.label}</p>
            </div>
            <div className="grid h-16 w-16 place-items-center rounded-full bg-primary text-lg font-extrabold text-primary-foreground" aria-label={`${readiness.score} pour cent`}>
              {readiness.score}%
            </div>
          </div>
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-surface-muted" aria-hidden="true">
            <div className="h-full rounded-full bg-primary transition-[width] duration-300 motion-reduce:transition-none" style={{ width: `${readiness.score}%` }} />
          </div>
          ...
          {readiness.suggestions.length > 0 ? (
            <div className="mt-5 border-t border-border/20 pt-4">
              <p className="text-xs font-extrabold">Pour l’améliorer</p>
              <ul className="mt-3 space-y-2">
                {readiness.suggestions.map((suggestion) => (
                  <li key={suggestion.key} className="flex items-start justify-between gap-3 text-xs">
                    <span className="text-muted-foreground">{suggestion.label}</span>
                    <span className="shrink-0 font-extrabold text-primary">+{suggestion.gain}%</span>
                  </li>
                ))}
              </ul>
            </div>
          ) : (
            <p className="mt-5 inline-flex items-center gap-2 rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300"><CheckCircle2 size={16} aria-hidden="true" /> Votre dossier est très complet.</p>
          )}
        </section>

        <section className={`${ui.surfaceMuted} mt-4 p-4`}>
          <p className="flex items-center gap-2 text-xs font-extrabold"><ShieldCheck size={16} className="text-primary" aria-hidden="true" /> Vos informations restent maîtrisées</p>
          <p className="mt-2 text-xs leading-5 text-muted-foreground">Les photos restent sur votre appareil dans ce LOT. Le téléphone n’est jamais rendu public automatiquement.</p>
        </section>

        <section className="mt-4 flex gap-2 rounded-xl border border-border/20 bg-card p-4 text-xs leading-5 text-muted-foreground">
          <Info size={16} className="mt-0.5 shrink-0 text-primary" aria-hidden="true" />
          <p>“Annonce prête” mesure uniquement la qualité des informations fournies. Ce n’est ni une estimation du prix, ni une garantie de vente.</p>
        </section>
      </aside>
    </div>
  );
