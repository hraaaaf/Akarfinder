...
function MediaImage({ item, className, onError, eager = false }: { item: PropertyMediaItem; className: string; onError: () => void; eager?: boolean }) {
  return (
    ...
    <img src={item.url} alt={item.alt} loading={eager ? "eager" : "lazy"} decoding="async" onError={onError} className={className} />
  );
}
...
export function PropertyMediaGallery({ listing, priceLabel, location, transactionLabel }: { listing: Listing; priceLabel: string; location: string; transactionLabel: string }) {
  ...
  const items = useMemo(() => model.items.filter((item) => !failedUrls.has(item.url)), [model.items, failedUrls]);
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Tab" && dialogRef.current) {
        const focusable = Array.from(dialogRef.current.querySelectorAll<HTMLElement>("button:not([disabled]), a[href]")).filter((element) => element.offsetParent !== null);
  ...
  const previous = () => { if (items.length > 1) setActiveIndex((current) => (current - 1 + items.length) % items.length); };
  const next = () => { if (items.length > 1) setActiveIndex((current) => (current + 1) % items.length); };
  ...
  const controls = (
    <div className="absolute right-3 top-3 z-20 flex gap-2 sm:right-4 sm:top-4">
      <button type="button" onClick={share} aria-label="Partager cette annonce" className="grid h-11 min-w-11 place-items-center rounded-full bg-white/95 px-3 text-[#0B2545] shadow-lg backdrop-blur transition hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE]"><Share2 size={19} strokeWidth={2} /><span className="sr-only">{shareState === "copied" ? "Lien copié" : "Partager"}</span></button>
      <div className="grid h-11 min-w-11 place-items-center rounded-full bg-white/95 shadow-lg backdrop-blur"><FavoriteToggleButton listingId={listing.id} variant="icon" /></div>
    </div>
  );
    ...
    return (
      <section data-property-media-mode="provider_preview" className="relative overflow-hidden rounded-[1.6rem] border border-[#eadfca] bg-white shadow-[0_18px_54px_rgba(7,27,51,0.16)]">
        <div className="relative h-[280px] sm:h-[460px]">
          <DbProviderThumbnail listing={listing} thumbnailUrl={model.items[0].url} className="absolute inset-0 h-full w-full" imgClassName="absolute inset-0 h-full w-full object-cover" />
  ...
  return (
    <>
      <section data-property-media-mode={realGallery ? "gallery" : "single_real"} className="relative overflow-hidden rounded-[1.6rem] border border-[#eadfca] bg-white shadow-[0_18px_54px_rgba(7,27,51,0.16)]">
        <div className="relative lg:hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          <div className="relative h-[320px] sm:h-[500px]">
            ...
            {controls}
            <HeroLabels listing={listing} transactionLabel={transactionLabel} priceLabel={priceLabel} location={location} />
            {realGallery ? <GalleryNav count={items.length} activeIndex={activeIndex} previous={previous} next={next} open={() => openFullscreen()} /> : (
              <button type="button" onClick={() => openFullscreen()} className="absolute bottom-4 right-4 z-20 grid h-11 w-11 place-items-center rounded-full bg-white text-[#0B2545] shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE]" aria-label="Agrandir la photo"><Maximize2 size={18} /></button>
            )}
            {model.attribution ? <span className="absolute bottom-16 right-4 z-10 max-w-[60%] rounded-full bg-black/50 px-2.5 py-1 text-right text-[10px] font-semibold text-white/80 backdrop-blur-sm">{model.attribution}</span> : null}
          </div>
        </div>

        {realGallery ? (
          <div className="relative hidden h-[500px] gap-1.5 bg-slate-100 lg:grid lg:grid-cols-[1.55fr_1fr]">
            <button type="button" onClick={() => openFullscreen(0)} className="relative overflow-hidden text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B63CE]" aria-label="Ouvrir la galerie en plein écran">
            ...
            <div className="grid min-w-0 grid-rows-2 gap-1.5">
              {[items[1], items[2]].map((item, index) => item ? (
                <button key={item.id} type="button" onClick={() => openFullscreen(index + 1)} className="relative overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B63CE]" aria-label={`Ouvrir la photo ${index + 2}`}><MediaImage item={item} className="absolute inset-0 h-full w-full object-cover transition duration-300 hover:scale-[1.02]" onError={() => fail(item.url)} /></button>
              ) : null)}
            </div>
            {controls}
            {model.attribution ? <span className="absolute bottom-16 right-4 z-20 max-w-[45%] rounded-full bg-black/50 px-2.5 py-1 text-right text-[10px] font-semibold text-white/80 backdrop-blur-sm">{model.attribution}</span> : null}
            <button type="button" onClick={() => openFullscreen()} className="absolute bottom-4 right-4 z-20 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 text-[12px] font-extrabold text-[#0B2545] shadow-lg transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE]"><Images size={17} /> Voir les {items.length} photos</button>
          </div>
        ) : (
          <div className="relative hidden h-[500px] lg:block">
            ...
            {controls}
            <HeroLabels listing={listing} transactionLabel={transactionLabel} priceLabel={priceLabel} location={location} />
            {model.attribution ? <span className="absolute bottom-16 right-4 z-10 max-w-[45%] rounded-full bg-black/50 px-2.5 py-1 text-right text-[10px] font-semibold text-white/80 backdrop-blur-sm">{model.attribution}</span> : null}
            <button type="button" onClick={() => openFullscreen()} className="absolute bottom-4 right-4 z-20 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-4 text-[12px] font-extrabold text-[#0B2545] shadow-lg transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE]" aria-label="Agrandir la photo"><Maximize2 size={17} /> Agrandir</button>
          </div>
        )}
      </section>

      {fullscreen && active ? (
        <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={`Galerie photos de ${listing.title}`} className="fixed inset-0 z-[100] flex flex-col bg-[#061425]/95 text-white backdrop-blur-sm">
          <div className="flex min-h-16 items-center justify-between gap-4 px-4 sm:px-6">
            ...
            <div className="flex gap-2">
              ...
              <button ref={closeRef} type="button" onClick={closeFullscreen} className="grid h-11 w-11 place-items-center rounded-full bg-white/10 hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" aria-label="Fermer la galerie"><X size={22} /></button>
            </div>
          </div>
          <div className="relative flex min-h-0 flex-1 items-center justify-center px-3 pb-4 sm:px-16">
            ...
            {items.length > 1 ? <><button type="button" onClick={previous} className="absolute left-3 grid h-12 w-12 place-items-center rounded-full bg-black/45 hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:left-5" aria-label="Photo précédente"><ChevronLeft size={26} /></button><button type="button" onClick={next} className="absolute right-3 grid h-12 w-12 place-items-center rounded-full bg-black/45 hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:right-5" aria-label="Photo suivante"><ChevronRight size={26} /></button></> : null}
          </div>
