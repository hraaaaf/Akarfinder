...
for (const slug of expectedCities) {
  if (!registry.includes(`slug: \"${slug}\"`)) {
    throw new Error(`missing premium city registry row: ${slug}`);
  }
}

const intelligenceProviderRows = registry
  .split("\n")
  .filter((line) => line.includes("marketIntelligenceProvider:") && line.includes("slug:"));
const enabledProviders = intelligenceProviderRows.filter((line) => !line.includes("marketIntelligenceProvider: null"));
...
if (!geoApiEntries.includes("casablanca-arrondissements")) {
  throw new Error("missing Casablanca territorial endpoint");
}
if (!geoApiEntries.includes("rabat-market-intelligence")) {
  throw new Error("missing Rabat market-intelligence endpoint");
}
for (const slug of ["casablanca", "marrakech", "tanger", "agadir", "fes"]) {
  if (geoApiEntries.includes(`${slug}-market-intelligence`)) {
    throw new Error(`unexpected unvalidated market-intelligence endpoint: ${slug}`);
  }
}
...
console.log(JSON.stringify({
  ...
  casablanca: {
    geometryStatus: "shadow/canary",
    featureCount: casaAudit.featureCount,
