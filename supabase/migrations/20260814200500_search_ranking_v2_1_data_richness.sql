-- SEARCH-RANKING-V2.1 — keep business lanes and relevance dominant,
-- but favor richer usable representations at comparable relevance.
-- Missing price never excludes a listing; it only receives less completeness boost.
...
begin
  ...
  with params as (
    ...
    select
      ...
      (
        (case when d.normalized_city is not null then 0.015 else 0 end)
        + (case when d.normalized_property_type is not null then 0.02 else 0 end)
        + (case when d.normalized_intent is not null then 0.015 else 0 end)
        + (case when d.normalized_price_mad is not null then 0.06 else 0 end)
        + (case when d.normalized_surface_m2 is not null then 0.04 else 0 end)
...
'Ranking v2.1: business lanes first; then relevance, quality, freshness and data richness with stronger price/surface completeness; exact URL dedup and bounded source-diversity penalty.';
