...
function accepted(sourceDomain: string, canonicalUrl: string, providers = ["openserp"]): UniversalCandidatePromotionRow {
  return {
    ...
    snippet: "Appartement 90 m2 à Rabat contact +212661590451",
...
test("M4 persisted seed is minimal and carries no discovery text", () => {
  const row = accepted("milkiya.ma", "https://milkiya.ma/fr_fr/Bien/location-bureau-86-m-a-casablanca", ["serper_mass_harvest", "openserp"]);
  const seed = projectM4MinimalSeed(row);
  assert.equal(seed.seed_provider, "serper_mass_harvest");
  assert.equal(seed.metadata, null);
  const serialized = JSON.stringify(seed);
  assert.equal(serialized.includes("Appartement à vendre à Rabat"), false);
  assert.equal(serialized.includes("Appartement 90 m2"), false);
  assert.equal(serialized.includes("appartement rabat"), false);
});
...
test("M4 preserves existing seeds and inserts only native net-new", () => {
  ...
  if (plan[1]?.action === "INSERT_NATIVE") {
    assert.equal(plan[1].seed.seed_provider, "serper_mass_harvest");
    assert.equal(plan[1].seed.metadata, null);
  }
});
