...
test("expired registry, missing sitemap or excessive drift fail closed", () => {
  const decision = evaluatePromotionBoundary({
    registryEligible: true,
    registryReviewStatus: "expired",
    sitemapSignalPresent: false,
    requestedBatchSize: 50,
    cumulativeAppliedRows: 0,
    candidateRows: 100,
    driftedRows: 5,
...
test("batch size and cumulative cap are hard gates", () => {
  assert.equal(evaluatePromotionBoundary({
    registryEligible: true,
    registryReviewStatus: "current",
    sitemapSignalPresent: true,
    requestedBatchSize: 101,
    cumulativeAppliedRows: 0,
    candidateRows: 5564,
    driftedRows: 0,
  }).allowed, false);
...
test("updated_at is captured but intentionally not rolled back", () => {
  ...
  const row = snapshotPromotionRow({
    canonicalUrl: "https://daragadir.com/x",
