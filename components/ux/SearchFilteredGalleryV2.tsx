...
function searchHref(values: SearchLinkValues, overrides: Partial<SearchLinkValues> = {}) {
  const next = { ...values, ...overrides };
  const params = new URLSearchParams();
  const offset = Math.max(0, (next.page - 1) * next.perPage);

  if (next.query) params.set("q", next.query);
  if (next.city) params.set("city", next.city);
  if (next.propertyType) params.set("property_type", next.propertyType);
  if (next.transactionType) params.set("transaction_type", next.transactionType);
  if (next.minPrice != null) params.set("min_price", String(next.minPrice));
  if (next.maxPrice != null) params.set("max_price", String(next.maxPrice));
  if (next.minSurface != null) params.set("min_surface", String(next.minSurface));
  if (next.maxSurface != null) params.set("max_surface", String(next.maxSurface));
...
function PreservedFilters({
  ...
  return (
    <>
      ...
      {transactionType ? <input type="hidden" name="transaction_type" value={transactionType} /> : null}
...
function PropertyTypeRail({ values }: { values: SearchLinkValues }) {
  return (
    <div className="border-b border-[#DCE8F5] bg-white">
      <div className="mx-auto max-w-[1280px] px-4 py-4 sm:px-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[.16em] text-blue-700">Types de biens</p>
            <p className="mt-1 text-xs font-semibold text-slate-500">Les visuels validés restent liés à la recherche.</p>
          </div>
          {values.propertyType ? (
            <a
              href={searchHref(values, { propertyType: undefined, page: 1 })}
        ...
        <div className="mt-3 flex snap-x gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {OPTION_A_PROPERTY_TYPES.map((item) => {
            const active = values.propertyType === item.value;
            return (
              <a
                key={item.value}
                href={searchHref(values, { propertyType: item.value, page: 1 })}
                aria-current={active ? "true" : undefined}
                className={`group min-w-[124px] snap-start overflow-hidden rounded-2xl border bg-white p-2 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
              ...
              >
                ...
                <p className={`px-1 pb-1 pt-2 text-[11px] font-extrabold ${active ? "text-blue-700" : "text-slate-700"}`}>
                  {item.pluralLabel}
                </p>
              </a>
            );
...
function SearchPagination({ values, totalPages }: { values: SearchLinkValues; totalPages: number }) {
  ...
  const baseClass = "grid h-10 min-w-10 place-items-center rounded-xl border px-3 text-sm font-extrabold transition";
          ...
          <div className="mx-auto max-w-[940px]">
            <header className="mb-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_8px_28px_rgba(15,23,42,.045)] sm:p-6">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-xs font-black uppercase tracking-[.16em] text-blue-700">Résultats AkarFinder</p>
                ...
                <div className="shrink-0">
                  <p className="mb-2 text-[11px] font-black uppercase tracking-[.12em] text-slate-400">Annonces par page</p>
            ...
            {visible.length ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center shadow-sm">
                ...
                <a href="/search?per_page=10" className="mt-5 inline-flex rounded-xl bg-blue-700 px-5 py-3 text-sm font-extrabold text-white">
