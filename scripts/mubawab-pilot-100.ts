...
const outputDir = join(__dirname, "..", "data-ingestion", "runs", "mubawab", "lot4-pilot-100");
...
type PilotError = { stage: string; url: string | null; source_id: string | null; message: string; retryable: boolean };
type PilotManifest = {
  run_id: string;
  source: "mubawab";
  started_at: string;
...
async function initialize() {
  ...
  if (discovery.manifest.pages_failed > 0) throw new Error(`discovery_failed_pages:${discovery.manifest.pages_failed}`);
  const candidates = discovery.listings.slice(0, MAX_DETAIL_FETCHES);
  if (candidates.length < MAX_DETAIL_FETCHES) throw new Error(`insufficient_candidates_for_pilot100:${candidates.length}`);

  const manifest: PilotManifest = {
    run_id: `mubawab-lot4-pilot100-${startedAt.replace(/[:.]/g, "-")}`,
    source: "mubawab",
...
async function processBatch(limit: number) {
  ...
  while (checkpoint.next_index < candidates.length && processedThisPass < limit) {
    ...
    if (checkpoint.processed_source_ids.includes(ref.source_id)) throw new Error(`checkpoint_duplicate:${ref.source_id}`);
    ...
    try {
      ...
      if (reasons.length === 0) manifest.listings_normalized += 1;
        ...
        const rejection: PilotError = { stage: "quality", url: ref.url, source_id: ref.source_id, message: reasons.join(","), retryable: false };
...
async function main() {
  ...
  if (checkpoint.next_index !== MAX_DETAIL_FETCHES) throw new Error(`pilot100_checkpoint_mismatch:${checkpoint.next_index}`);
