...
const ACTION_COPY: Record<SellerPublicationAction, { title: string; detail: string; button: string }> = {
  publish: { title: "Mettre cette annonce en ligne ?", detail: "Elle deviendra visible après votre confirmation.", button: "Confirmer la mise en ligne" },
  pause: { title: "Mettre cette annonce en pause ?", detail: "Elle ne sera plus visible jusqu’à ce que vous la remettiez en ligne.", button: "Confirmer la pause" },
  resume: { title: "Remettre cette annonce en ligne ?", detail: "Elle redeviendra visible après votre confirmation.", button: "Confirmer la remise en ligne" },
  withdraw: { title: "Retirer définitivement cette annonce ?", detail: "Cette action ne pourra pas être annulée depuis ce dossier.", button: "Confirmer le retrait" },
};
...
export function SellerPublicationPanel({ draftId, uploadToken }: Props) {
  ...
  const [pendingAction, setPendingAction] = useState<SellerPublicationAction | null>(null);
  ...
  useEffect(() => {
    void refresh().catch((cause) => setError(cause instanceof Error ? cause.message : "Le statut n’est pas disponible."));
  }, [refresh]);
  ...
  const confirmation = pendingAction ? ACTION_COPY[pendingAction] : null;
  ...
  return (
    <section className={`${ui.surface} mt-6 p-5 text-left`} aria-labelledby="seller-publication-title">
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        {publication.status === "unpublished" ? <button type="button" disabled={busy} onClick={() => setPendingAction("publish")} className={ui.primaryAction}><Eye size={16} /> Mettre mon annonce en ligne</button> : null}
        {publication.status === "live" ? <><button type="button" disabled={busy} onClick={() => setPendingAction("pause")} className={ui.secondaryAction}><PauseCircle size={16} /> Mettre en pause</button><button type="button" disabled={busy} onClick={() => setPendingAction("withdraw")} className={ui.secondaryAction}><EyeOff size={16} /> Retirer l’annonce</button></> : null}
        {publication.status === "paused" ? <><button type="button" disabled={busy} onClick={() => setPendingAction("resume")} className={ui.primaryAction}><PlayCircle size={16} /> Remettre en ligne</button><button type="button" disabled={busy} onClick={() => setPendingAction("withdraw")} className={ui.secondaryAction}><EyeOff size={16} /> Retirer l’annonce</button></> : null}
      </div>

      {confirmation && pendingAction ? (
        <div role="dialog" aria-modal="true" aria-labelledby="seller-publication-confirmation-title" className="mt-5 rounded-2xl border border-border/30 bg-surface-muted p-4">
          <h3 id="seller-publication-confirmation-title" className="text-base font-extrabold">{confirmation.title}</h3>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">{confirmation.detail}</p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            <button type="button" disabled={busy} onClick={() => void act(pendingAction)} className={pendingAction === "withdraw" ? ui.secondaryAction : ui.primaryAction}>{confirmation.button}</button>
            <button type="button" disabled={busy} onClick={() => setPendingAction(null)} className={ui.secondaryAction}>Annuler</button>
          </div>
        </div>
      ) : null}
