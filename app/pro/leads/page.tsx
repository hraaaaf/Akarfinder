...
export default async function LeadsPage({ searchParams }: Props) {
  ...
  try {
    ...
    if (filter === "buyer_profile") {
      q = q.eq("lead_type", "buyer_profile");
  ...
  const buyerProfiles = normalizedLeads.filter((lead) => lead.lead_type !== "visit_request");
      ...
      <Container className="py-10">
        ...
        <div className="mt-10 rounded-xl border border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mt-3 text-[12px] text-amber-700/90">
            Dossiers acheteurs : {buyerProfiles.length} · Demandes de visite : {visitRequests.length}
          </p>
        </div>
      </Container>
    </div>
