...
async function request(item) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/search?${toSearchParams(item.query)}`, {
      ...
      headers: { "user-agent": "AkarFinder-ODM-Canary-25-Certification/1.0" },
    });
...
async function visibleProbe(item) {
  ...
  try {
    const response = await fetch(`${BASE_URL}/search?${toSearchParams(item.query)}`, {
      signal: controller.signal,
      headers: { "user-agent": "AkarFinder-ODM-Canary-25-Page-Probe/1.0" },
    });
...
async function main() {
  ...
  const report = {
    ...
    gates: {
      ...
      bucket_rate_near_twenty_five_percent: rate >= 0.235 && rate <= 0.265,
  ...
  await writeFile("artifacts/odm-canary-25-production-certification-v1.json", `${JSON.stringify(report, null, 2)}\n`);
  const pass = Object.values(report.gates).every(Boolean);
  const summary = [
    "# ODM Canary 25% Production Certification V1",
    "",
