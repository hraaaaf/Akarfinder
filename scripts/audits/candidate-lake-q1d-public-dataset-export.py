...
DATASETS=[
 {'name':'realestatebuddy_mubawab','source_domain':'mubawab.ma','transaction':None,'price_is_mad':False,
 ...
 {'name':'hicham_mubawab_sale','source_domain':'mubawab.ma','transaction':'buy','price_is_mad':True,
 ...
 {'name':'hicham_avito_rent','source_domain':'avito.ma','transaction':'rent','price_is_mad':True,
  'commit':'ad204e00ba8df9ee5a78b7ae6366709cd070d567',
 ...
 {'name':'hicham_avito_sale','source_domain':'avito.ma','transaction':'buy','price_is_mad':True,
  'commit':'ad204e00ba8df9ee5a78b7ae6366709cd070d567',
...
ALIASES={
 ...
 'city':['city','City','ville','Ville'], 'district':['neighborhood','quartier','district','zone'],
 'price':['Prix','prix','price','Price','prix_mad'], 'currency':['Devise','devise','currency','currency_code'],
 ...
 'title':['title','titre','Title','Titre'], 'url':['url','URL','lien','link'],
 'address_text':['location','adresse','address','localisation'],
}
...
def pick(row,names):
 ...
 for n in names:
  candidates=[]
  if n in direct:candidates.append(direct[n])
  if norm_header(n) in folded:candidates.append(folded[norm_header(n)])
  for v in candidates:
...
def mad_price(rec,ds):
 ...
 c=re.sub(r'[^a-z0-9]+','',cur.lower())
 return value if c in {'dh','dhs','mad'} else None
...
def download(url):
 req=urllib.request.Request(url,headers={'User-Agent':'AkarFinder-Q1D-public-dataset-export/1.1'})
 with urllib.request.urlopen(req,timeout=90) as r:return r.read()

rows=[]; dataset_summaries=[]
for ds in DATASETS:
 raw=download(ds['url']); digest=hashlib.sha256(raw).hexdigest(); text=raw.decode('utf-8-sig',errors='replace')
 reader=csv.DictReader(io.StringIO(text)); seen=usable=0
 for rec in reader:
  seen+=1; sid=source_id(pick(rec,ALIASES['source_id']))
  if not sid:continue
  usable+=1
  rows.append({
   'source_domain':ds['source_domain'],'source_id':sid,'identity_key':f"{ds['source_domain']}|id:{sid}",
   ...
   'price_mad':mad_price(rec,ds),'surface_m2':positive(pick(rec,ALIASES['surface_m2'])),
   'rooms_count':integer(pick(rec,ALIASES['rooms_count'])),'bedrooms_count':integer(pick(rec,ALIASES['bedrooms_count'])),
   'bathrooms_count':integer(pick(rec,ALIASES['bathrooms_count'])),'latitude':geo(pick(rec,ALIASES['latitude']),-90,90),
   'longitude':geo(pick(rec,ALIASES['longitude']),-180,180),'title':txt(pick(rec,ALIASES['title'])),
   'address_text':txt(pick(rec,ALIASES['address_text'])),'url':txt(pick(rec,ALIASES['url']))})
