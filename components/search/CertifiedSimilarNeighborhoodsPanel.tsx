    ...
    <aside className="overflow-hidden rounded-2xl border border-border/15 bg-card shadow-[0_14px_40px_rgba(2,10,24,0.12)] dark:border-white/10 dark:bg-white/[0.045]">
      <div className="border-b border-border/12 bg-surface/70 px-5 py-4 dark:border-white/8 dark:bg-white/[0.03]">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-[1.05rem] font-extrabold tracking-[-0.02em] text-foreground">Proximité descriptive certifiée</h2>
          </div>
      ...
      {model.status === "available" ? (
        <div className="space-y-4 px-5 py-5">
          <div>
            <p className="text-[11px] font-bold text-muted-foreground">Quartier de référence</p>
            <p className="mt-1 text-[15px] font-extrabold text-foreground">{model.selectedNeighborhood}</p>
            <p className="mt-1 text-[12px] text-muted-foreground">{model.selectedPublishedPricePerM2?.toLocaleString("fr-MA")} MAD/m² publié</p>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {model.candidates.map((candidate) => (
              <article key={candidate.key} className="rounded-xl border border-border/12 bg-surface/60 p-4 dark:border-white/8 dark:bg-white/[0.03]">
                ...
                <p className="mt-1 text-[10.5px] text-muted-foreground">Écart descriptif : {candidate.priceGapPct.toLocaleString("fr-MA", { maximumFractionDigits: 1 })} %</p>
                <p className="mt-2 text-[10.5px] text-muted-foreground">{candidate.visibleCanonicalProperties.toLocaleString("fr-MA")} propriété(s) canonique(s) visible(s)</p>
        ...
        <div className="px-5 py-5">
          <p className="text-[13px] font-extrabold text-foreground">Similarités indisponibles</p>
