-- ODM-DISPLAY-POLICY-V2-PERFORMANCE-FIX
-- Replaces the expensive typed-surface view join with direct freshness assessment.
-- Shadow-only; no public, ranking, Thin Index or SERP mutation.
...
with base as (
  select
    a.observation_id,a.seed_id,a.canonical_url,a.source_domain,a.seed_provider,
    a.vertical_classification,a.quality_tier,a.quality_score,a.observation_title,
    a.observation_snippet,a.observation_observed_at,
    d.normalized_city,d.normalized_property_type,d.normalized_intent,
    d.normalized_price_mad,d.normalized_surface_m2,
    p.policy_resolution_status,p.resolved_registry_source_domain,
    p.display_policy as resolved_display_policy,
    p.no_bypass_required as resolved_no_bypass_required,
    coalesce(public.odm_audit_freshness_assessment_v2(
      a.observation_observed_at,a.seed_provider,a.observation_source,a.freshness_status
    ) #>> '{freshness_status_v2}','unknown') as freshness_status_v2,
    e.economic_status,e.principal_value_mad,
    r.recovery_status,r.recovered_value_mad,r.recovered_economic_type
  ...
  left join public.odm_economic_observation_state_shadow_v1 e
    on e.observation_id=a.observation_id and e.parser_version='odm_economic_parser_v2'
  left join public.odm_economic_recovery_candidate_shadow_v1 r on r.observation_id=a.observation_id
), assessed as (
  select b.*,
    (nullif(btrim(b.canonical_url),'') is not null and
      (nullif(btrim(b.observation_title),'') is not null or nullif(btrim(b.observation_snippet),'') is not null)) as has_exploitable_evidence,
    case
      ...
      when nullif(btrim(b.observation_title),'') is null and nullif(btrim(b.observation_snippet),'') is null then 'blocked'
      ...
      when b.normalized_city is null or b.normalized_property_type is null or b.normalized_intent is null then 'displayable_degraded'
  ...
  ],null) as decision_reasons_v2 from assessed a
)
select observation_id,seed_id,canonical_url,source_domain,seed_provider,vertical_classification,
quality_tier,quality_score,observation_title,observation_snippet,observation_observed_at,
normalized_city,normalized_property_type,normalized_intent,normalized_price_mad,normalized_surface_m2,
policy_resolution_status,resolved_registry_source_domain,resolved_display_policy,resolved_no_bypass_required,
freshness_status_v2,economic_status,principal_value_mad,recovery_status,recovered_value_mad,
recovered_economic_type,has_exploitable_evidence,display_tier_v2,decision_reasons_v2,
false::boolean as publication_eligible,false::boolean as ranking_eligible,
