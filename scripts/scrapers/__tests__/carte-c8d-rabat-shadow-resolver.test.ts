...
test("C8D shadow resolves exact structured candidate districts without activating them", () => {
  for (const [district, expectedId] of [
    ...
    const result = resolveRabatLocalityShadow({ district });
...
test("C8D shadow resolves certified Ocean by canonical name and explicit accentless alias", () => {
...
test("C8D shadow prefers an exact structured district over incidental text", () => {
  const result = resolveRabatLocalityShadow({ district: "Aviation", title: "À quelques minutes d'Agdal" });
  assert.equal(result.status, "matched");
  if (result.status !== "matched") return;
  assert.equal(result.localityId, "candidate_rabat_aviation");
...
test("C8D shadow does not fuzzy-resolve generic Rabat or partial words", () => {
  assert.deepEqual(resolveRabatLocalityShadow({ district: "Rabat" }), { status: "unresolved", reason: "no_exact_locality_signal" });
...
test("C8D shadow keeps short alias Riad out of free-text matching but accepts it as structured exact", () => {
  assert.equal(resolveRabatLocalityShadow({ title: "Belle résidence Riad avec jardin" }).status, "unresolved");
  const structured = resolveRabatLocalityShadow({ district: "Riad" });
