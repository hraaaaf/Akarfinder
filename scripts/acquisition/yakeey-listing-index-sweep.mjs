...
const START = '/fr-ma/achat/biens/maroc';
const OUT = 'data/audits/raw-results/yakeey-listing-index-sweep.json';
const MAX_PAGES = 120;
...
function extractListings(html) {
  ...
  const patterns = [
    /https?:\/\/yakeey\.com\/(fr-ma\/acheter-[^"'<>\s?#]+-da\d+)/gi,
    /(?:href=["'])\/?(fr-ma\/acheter-[^"'<>?#]+-da\d+)(?:[?"'])/gi,
    /\/(fr-ma\/acheter-[^"'<>\s?#]+-da\d+)/gi,
  ];
