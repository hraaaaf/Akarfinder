...
const requestedDomain = process.env.Q1A_DATA49B_DOMAIN?.trim() || null;
if (requestedDomain && !DATA_4_9B_SOURCES.includes(requestedDomain as Domain)) throw new Error(`unknown DATA4.9B domain ${requestedDomain}`);
const TARGET_DOMAINS: readonly Domain[] = requestedDomain ? [requestedDomain as Domain] : DATA_4_9B_SOURCES;
  ...
  for (let attempt = 1; attempt <= 5; attempt += 1) {
    try {
      const response = await fetch(url, {
        headers: { 'user-agent': 'AkarFinder-Q1A-DATA49B-historical-union/1.1 metadata-only' },
...
async function queryDomain(index: string, domain: string): Promise<string[]> {
  ...
  const common = new URLSearchParams({ url: domain, matchType: 'domain', output: 'json', fl: 'url', filter: 'status:200', collapse: 'urlkey' });
  ...
  for (let page = 0; page < pages; page += 1) {
    ...
    for (const line of text.split('\n')) {
      ...
      } catch { /* malformed index rows are unusable evidence */ }
    }
  }
  return [...urls];
}

function classify(domain: Domain, raw: Set<string>) {
...
async function main(): Promise<void> {
  ...
  for (const domain of TARGET_DOMAINS) {
    ...
    for (const index of INDEXES) {
      ...
      progression.push({ index, indexRawUrls: found.length, queryComplete, cumulativeRawUrls: raw.size, cumulativeCandidateRows: candidates.length, expectedCandidateRows: EXPECTED[domain], deltaToExpected: candidates.length - EXPECTED[domain] });
    }
    const { buckets, candidates } = classify(domain, raw);
    const candidateLines = candidates.map((row) => `${row.identity}\t${row.canonicalUrls[0] ?? ''}`);
    for (const row of candidates) allManifest.push({ source_domain: domain, identity: row.identity, canonical_url: row.canonicalUrls[0] ?? '' });
    ...
    domains.push({ sourceDomain: domain, expectedHistoricalCandidateRows: EXPECTED[domain], distinctRawUrls: raw.size, distinctConservativeIdentities: buckets.size, candidateRows: candidates.length, deltaToExpected: candidates.length - EXPECTED[domain], firstExactAt, everReachedOrExceededExpected: progression.some((p) => Number(p.cumulativeCandidateRows) >= EXPECTED[domain]), queryComplete: errors.length === 0, errors, candidateDigestSha256: sha256(candidateLines.join('\n')), progression });
  }
  ...
  const expectedTargetTotal = TARGET_DOMAINS.reduce((sum, d) => sum + EXPECTED[d], 0);
  const summary = {
    schemaVersion: 'Q1A_DATA49B_COMMONCRAWL_HISTORICAL_UNION_V2', historicalRun: 31370449455, historicalArtifact: 9055869351,
    historicalArtifactSha256: 'df4f38102877a5de29a7980dbb7e5b32a4110813d8af132fc48a46cf87126520', historicalObservedAt: '2026-08-10T08:32:48.268Z',
    targetDomains: TARGET_DOMAINS, expectedHistoricalCounts: EXPECTED, indexes: INDEXES, readOnly: true, databaseWrites: 0, productionWrites: 0,
    sourceSiteFetches: 0, sourceContentFetches: 0, warcFetches: 0, commonCrawlUrlIndexRequestsOnly: true, vercelDeployments: 0,
    certificationState: 'EVIDENCE_ONLY_UNLESS_EXACT_IDENTITY_ORACLE_IS_RECOVERED', totalCandidateRows: allManifest.length, expectedHistoricalTotal: expectedTargetTotal,
    allQueriesComplete: domains.every((d) => d.queryComplete === true), allFinalCountsMatch: domains.every((d) => d.candidateRows === d.expectedHistoricalCandidateRows),
    allDomainsHitExactAtSomeHorizon: domains.every((d) => typeof d.firstExactAt === 'string'), manifestSha256: sha256(manifestText), domains,
  };
  await fs.writeFile(path.join(OUT, 'summary.json'), `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
  console.log(JSON.stringify({ targetDomains: summary.targetDomains, totalCandidateRows: summary.totalCandidateRows, expectedHistoricalTotal: summary.expectedHistoricalTotal, allQueriesComplete: summary.allQueriesComplete, allFinalCountsMatch: summary.allFinalCountsMatch, allDomainsHitExactAtSomeHorizon: summary.allDomainsHitExactAtSomeHorizon, manifestSha256: summary.manifestSha256, domains: domains.map((d) => ({ sourceDomain: d.sourceDomain, candidateRows: d.candidateRows, expected: d.expectedHistoricalCandidateRows, delta: d.deltaToExpected, firstExactAt: d.firstExactAt, queryComplete: d.queryComplete })) }, null, 2));
}
