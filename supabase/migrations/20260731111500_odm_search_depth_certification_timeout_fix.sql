-- ODM SEARCH DEPTH CERTIFICATION V1 TIMEOUT FIX
-- Bounds the Shadow certification window to 100 rows for reliable connected execution.
...
with assembled as materialized (
  ...
  select s.scenario_id,s.city,s.property_type,s.intent,s.dimension,
    count(a.*) as matched_rows,
    count(a.*) filter(where a.display_tier_v2='displayable_ranked') as ranked_rows,
    count(a.*) filter(where a.display_tier_v2='displayable_degraded') as degraded_rows,
    count(a.*) filter(where a.normalized_price_mad is not null) as price_rows,
    count(a.*) filter(where a.normalized_surface_m2 is not null) as surface_rows
  from scenarios s
...
select jsonb_build_object(
  ...
  'summary',jsonb_build_object(
    'geographic_scenarios_with_results',count(*) filter(where dimension='geographic' and matched_rows>0),
    'geographic_zero_result_scenarios',count(*) filter(where dimension='geographic' and matched_rows=0),
    'structured_scenarios_with_results',count(*) filter(where dimension='structured' and matched_rows>0),
    'structured_zero_result_scenarios',count(*) filter(where dimension='structured' and matched_rows=0),
    'scenarios_with_ranked_results',count(*) filter(where ranked_rows>0)
  ),
  'scenarios',jsonb_agg(jsonb_build_object(
    'scenario_id',scenario_id,'dimension',dimension,'city',city,
    'property_type',property_type,'intent',intent,'rows',matched_rows,
    'ranked',ranked_rows,'degraded',degraded_rows,'with_price',price_rows,'with_surface',surface_rows,
    'zero_result',matched_rows=0,
    'ranked_share_percent',round(100.0*ranked_rows/nullif(matched_rows,0),2),
    'price_coverage_percent',round(100.0*price_rows/nullif(matched_rows,0),2),
    'surface_coverage_percent',round(100.0*surface_rows/nullif(matched_rows,0),2)
...
comment on function public.odm_search_depth_certification_report_v1() is 'Bounded 100-row Shadow depth certification separating geographic coverage from structured type and intent depth.';
