  ...
  return (
    <article className="group grid overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_8px_28px_rgba(15,23,42,.055)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_38px_rgba(15,23,42,.1)] md:grid-cols-[238px_1fr]">
      <div className="relative min-h-[210px] overflow-hidden bg-slate-100">
        {image ? (
          <div
            className="h-full min-h-[210px] bg-cover bg-center transition duration-500 group-hover:scale-[1.025]"
            style={{ backgroundImage: `url(${image})` }}
          />
        ) : (
          <div className="grid h-full min-h-[210px] place-items-center bg-gradient-to-br from-slate-100 via-blue-50 to-slate-200 text-slate-400">
            <Building2 size={46} strokeWidth={1.4} />
          </div>
        )}
        {listing.can_show_gallery && listing.images_count ? (
          <span className="absolute bottom-3 left-3 rounded-lg bg-slate-950/75 px-2.5 py-1 text-[11px] font-extrabold text-white">
      ...
      <div className="flex min-w-0 flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-[24px] font-black tracking-[-.045em] text-slate-950">
              {formatPrice(listing.price)}
            </p>
            <h2 className="mt-1 line-clamp-2 text-[16px] font-extrabold leading-6 text-slate-800">
              {listing.title}
            </h2>
            <p className="mt-1 flex items-center gap-1 text-[13px] font-medium text-slate-500">
              <MapPin size={14} />
              {[listing.neighborhood, listing.city].filter(Boolean).join(", ")}
            </p>
          </div>
          {info ? (
            <span className="max-w-[150px] shrink-0 rounded-xl bg-slate-100 px-3 py-2 text-center text-[10px] font-extrabold leading-4 text-slate-700">
              {info}
            </span>
          ) : null}
        </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="rounded-lg bg-slate-100 px-3 py-1.5 text-[11px] font-extrabold text-slate-700">
          ...
          {listing.duplicate_group_id ? (
            <span className="rounded-lg bg-blue-50 px-3 py-1.5 text-[11px] font-extrabold text-blue-700">
        ...
        <div className="mt-auto flex flex-wrap items-end justify-between gap-3 border-t border-slate-100 pt-4 text-[11px] text-slate-500">
          ...
          {canOpen ? (
            <a
              ...
              className="flex items-center gap-1.5 text-[12px] font-extrabold text-blue-700 hover:underline"
            >
