...
async function latestIndexes(){const r=await fetch('https://index.commoncrawl.org/collinfo.json',{signal:AbortSignal.timeout(8000)});if(!r.ok)throw new Error(`collinfo ${r.status}`);return (await r.json()).slice(0,2).map(x=>x.id)}
async function findRecord(url,indexes){for(const idx of indexes){try{const q=`https://index.commoncrawl.org/${idx}-index?url=${encodeURIComponent(url)}&output=json&filter=status:200&filter=mime:text/html&collapse=urlkey`;const r=await fetch(q,{signal:AbortSignal.timeout(6000)});if(!r.ok)continue;const text=await r.text();const lines=text.split('\n').filter(Boolean);if(lines.length){try{return JSON.parse(lines[0])}catch{}}}catch{}}return null}
