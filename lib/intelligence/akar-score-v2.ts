...
const LIMITATIONS = [
  "AkarScore mesure la qualité de documentation disponible, pas la vérité des informations ni un niveau de sûreté.",
...
function traceabilityScore(channel: FreshnessVerificationChannel): number | null {
  switch (channel) {
...
function traceabilityExplanation(channel: FreshnessVerificationChannel): string {
  switch (channel) {
    ...
    case "search_discovery": return "Signal découvert via un canal de recherche, avec une traçabilité plus limitée qu'une source structurée directe.";
...
function marketScore(market: MarketIntelligenceV2): number | null {
...
function marketExplanation(market: MarketIntelligenceV2): string {
  if (market.status !== "evaluated") return "Aucun contexte marché suffisamment compatible n'est disponible pour noter cette dimension.";
...
function coherenceExplanation(anomaly: AnomalyEngineV1): string {
  if (anomaly.status !== "evaluated" || anomaly.anomaly_score == null) {
    return "Les contrôles de cohérence ne couvrent pas suffisamment cette annonce pour noter cette dimension.";
  }
  if (anomaly.signals.length === 0) {
    return "Aucun signal inhabituel n'a été déclenché par les contrôles exécutables ; les contrôles indisponibles restent indéterminés.";
  }
  return `${anomaly.signals.length} signal${anomaly.signals.length === 1 ? "" : "aux"} inhabituel${anomaly.signals.length === 1 ? "" : "s"} affecte${anomaly.signals.length === 1 ? "" : "nt"} uniquement la composante de cohérence.`;
}

function corroborationBonus(multisource: MultiSourcePropertyIntelligenceV1): AkarScoreBonusV2 {
...
function publicLabel(score: number | null): string {
...
function evidence(input: AkarScoreV2Input): AnalysisEvidenceV1[] {
  const items: AnalysisEvidenceV1[] = [
  ...
  if (city.value != null && city.visibility === "PUBLIC") {
    items.push(evidenceFromFact(`${input.property.property_id}:akar-score:city`, city));
  }
  if (input.selected_offer?.title.value != null && input.selected_offer.title.visibility === "PUBLIC") {
    items.push(evidenceFromFact(`${input.selected_offer.offer_id}:akar-score:title`, input.selected_offer.title));
  }
  if (input.selected_offer?.price_amount.value != null && input.selected_offer.price_amount.visibility === "PUBLIC") {
    items.push(evidenceFromFact(`${input.selected_offer.offer_id}:akar-score:price`, input.selected_offer.price_amount));
  }
  const seen = new Set<string>();
  return items.filter((item) => item.public_usable && !seen.has(item.evidence_id) && seen.add(item.evidence_id));
}
...
function unavailable(
  ...
  const claim = createUnavailableClaim({
    claim_id: `akar-score:${propertyId}`,
...
export function evaluateAkarScoreV2(input: AkarScoreV2Input): AkarScoreV2 {
  ...
  const provenance = traceabilityScore(input.freshness.verification_channel);
  const coherence = coherenceScore(input.anomaly);
  const market = marketScore(input.market);

  const raw: RawComponent[] = [
    {
      key: "completeness", label: "Complétude utile", weight: 30, status: "evaluated",
      score: clamp(input.completeness.score),
    ...
    {
      key: "provenance_traceability", label: "Traçabilité et provenance", weight: 25,
      status: provenance == null ? "unavailable" : "evaluated", score: provenance,
      explanation: traceabilityExplanation(input.freshness.verification_channel),
    },
    {
      key: "freshness", label: "Fraîcheur d'observation", weight: 15,
    ...
    {
      key: "data_coherence", label: "Cohérence des données", weight: 15,
      status: coherence == null ? "unavailable" : "evaluated", score: coherence,
      explanation: coherenceExplanation(input.anomaly),
    },
    {
      key: "market_context_quality", label: "Qualité du contexte marché", weight: 15,
      status: market == null ? "unavailable" : "evaluated", score: market,
      explanation: marketExplanation(input.market),
    },
  ];

  const available = raw.filter((component) => component.status === "evaluated" && component.score != null);
  ...
  const components: AkarScoreComponentV2[] = raw.map((component) => {
    ...
    return {
      ...component,
      normalized_weight: round1((component.weight / availableWeight) * 100),
      contribution: round1((component.score * component.weight) / availableWeight),
    };
  });

  if (!coverage.minimum_coverage_met) {
    return unavailable(input.property.property_id, components, coverage, generatedAt);
  }

  const baseScore = round1(
    available.reduce((sum, component) => sum + Number(component.score) * component.weight, 0) / availableWeight,
  );
  const bonus = corroborationBonus(input.multisource);
  const finalScore = Math.round(clamp(baseScore + bonus.multi_source_corroboration));
  const claimEvidence = evidence(input);
  ...
  const claim: AnalysisClaimV1 = {
    ...
    domain: "information_quality",
    label: `AkarScore ${finalScore}/100 — ${publicLabel(finalScore)}`,
  ...
  return {
    ...
    public_label: publicLabel(finalScore),
    measured_as: "information_documentation_quality",
    components,
    coverage,
    bonus,
