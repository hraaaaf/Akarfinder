...
import { SITEMAP_FRESHNESS_TTL_DAYS, type CanarySeedState } from "../data4/daragadir-freshness-evidence-canary";
import { WRITE_CANARY_RUN_ID, WRITE_CANARY_SIZE, buildWriteCanaryPlan, selectWriteCanary } from "../data4/daragadir-freshness-write-canary";

const outDir = process.env.DATA_4_3E_OUT_DIR ?? ".tmp/data-4-3e/results";
const PAGE_SIZE = 1000;
const TIMEOUT_MS = 15000;
const MAX_SOURCE_REQUESTS = 40;
const SEED_LOOKUP_CHUNK_SIZE = 10;
let sourceRequests = 0;

function env(name: string): string {
  const value = process.env[name];
  if (!value) throw new Error(`DATA-4.3E requires ${name}`);
  return value;
}
...
async function fetchAllowedText(urlString: string): Promise<string> {
  ...
  const response = await fetch(urlString, {
    redirect: "follow",
    headers: { "user-agent": "AkarFinder/1.0 (+freshness-write-canary-dry-run; sitemap-only; no-detail-fetch)" },
    signal: AbortSignal.timeout(TIMEOUT_MS),
  });
...
async function main(): Promise<void> {
  ...
  const selected = selectWriteCanary(eligibleSeedOnly);
  if (selected.length !== WRITE_CANARY_SIZE) throw new Error(`Expected ${WRITE_CANARY_SIZE} rows, got ${selected.length}`);

  const seeds = await loadSeedStates(selected.map((row) => row.canonicalUrl));
  if (seeds.length !== WRITE_CANARY_SIZE) throw new Error(`Expected ${WRITE_CANARY_SIZE} seed states, got ${seeds.length}`);
  ...
  const manifest = selected.map((candidate) => {
    ...
    return buildWriteCanaryPlan(before, {
  ...
  const proof = {
    schemaVersion: "data-4-3e-first-freshness-write-canary-v1",
    generatedAt,
    mode: "DRY_RUN",
    runId: WRITE_CANARY_RUN_ID,
