...
const externalResults = [
  ...
  },
  {
    ...
    source_id: "fixture-source-4",
    source_name: "Source Maroc",
    domain: "example.ma",
    result_origin: "public_sitemap",
    search_result_display_mode: "thin_indexed_result",
    source_badge: "public_indexed",
  ...
  for (const viewport of viewports) {
    ...
    try {
      ...
      const metrics = await page.evaluate(() => {
        ...
        const provenanceIndex = childTexts.findIndex((value) => value.includes("Source externe"));
        const actionIndex = childTexts.findIndex((value) => value.includes("Voir sur le site d’origine"));
      ...
      if (!metrics.unknownText.includes("Source externe")) throw new Error(`${viewport.name}: external provenance is not visible`);
