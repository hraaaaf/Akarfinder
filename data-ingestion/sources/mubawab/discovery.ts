...
export type DiscoveryRoute = { city: string; category_key: string; page: number; url: string };
export type DiscoveredListingRef = { source_id: string; url: string; route_url: string; detail_family: "a" | "pa" };
export type DiscoveryManifest = {
  ...
  routes: Array<DiscoveryRoute & { status: "ok" | "error"; discovered: number; unique_added: number; error?: string }>;
};

function enc(value: string) { return encodeURIComponent(value).replace(/%2F/gi, "/"); }

export function buildDiscoveryRoutes(
  maxPages = config.safety.max_pages_per_combo_default,
  filters: { city?: string; category_key?: string } = {},
): DiscoveryRoute[] {
  const routes: DiscoveryRoute[] = [];
  for (const city of config.cities) {
    if (filters.city && city.name !== filters.city) continue;
    for (const category of config.categories) {
      if (!category.enabled || !category.st_slug) continue;
      if (filters.category_key && category.key !== filters.category_key) continue;
      for (let page = 1; page <= maxPages; page++) {
        const suffix = page === 1 ? "" : `:p:${page}`;
        routes.push({ city: city.name, category_key: category.key, page, url: `${config.base_url}/fr/st/${enc(city.slug)}/${category.st_slug}${suffix}` });
      }
    }
  }
  return routes;
...
export async function runDiscovery(
  ...
  options: { maxPages?: number; city?: string; category_key?: string; now?: () => string } = {},
): Promise<{ manifest: DiscoveryManifest; listings: DiscoveredListingRef[] }> {
  const routes = buildDiscoveryRoutes(options.maxPages, { city: options.city, category_key: options.category_key });
  const globalById = new Map<string, DiscoveredListingRef>();
  let duplicateRefs = 0, pagesSucceeded = 0, pagesFailed = 0;
  ...
  for (const route of routes) {
    try {
      ...
      for (const ref of refs) {
        ...
        else { globalById.set(ref.source_id, ref); uniqueAdded++; }
      }
      pagesSucceeded++;
      routeReports.push({ ...route, status: "ok", discovered: refs.length, unique_added: uniqueAdded });
    } catch (error) {
      pagesFailed++;
      routeReports.push({ ...route, status: "error", discovered: 0, unique_added: 0, error: error instanceof Error ? error.message : String(error) });
    }
  }
