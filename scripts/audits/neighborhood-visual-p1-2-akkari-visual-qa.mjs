...
const outputDir = `data/audits/neighborhood-visual-p1-2-akkari/${variant}`;
const viewports = [
...
const expectedIds = ["rabat-akkari-signature-v1", "rabat-akkari-immobilier-v1", "rabat-akkari-lifestyle-v1"];
const expectedSourceDimensions = [
  { path: "/__qa/akkari-signature.jpg", width: 3000, height: 4000 },
  { path: "/__qa/akkari-immobilier.jpg", width: 1280, height: 720 },
  { path: "/__qa/akkari-lifestyle.jpg", width: 1280, height: 720 },
];
...
async function readMetrics(page) {
  return page.evaluate(({ expectedRoles }) => {
    ...
    const cards = [...document.querySelectorAll("[data-akkari-qa-card]")];
    return {
      cards: cards.map((card) => {
        ...
        return {
          role: card.getAttribute("data-scene-role"), id: card.getAttribute("data-visual-id"), sourceKind: card.getAttribute("data-source-kind"),
          ...
          detailText, detailCount: detailText ? detailText.split("•").length : 0,
          ...
          credit: text(card.querySelector("[data-akkari-qa-credit]")),
      ...
      }), expectedRoles, clientWidth: document.documentElement.clientWidth, scrollWidth: document.documentElement.scrollWidth,
    };
  }, { expectedRoles });
}
...
try {
  for (const viewport of viewports) {
    ...
    try {
      const response = await page.goto(`${baseUrl}/visual-qa/akkari`, { waitUntil: "networkidle", timeout: 45_000 });
      if (!response || response.status() >= 400) throw new Error(`${viewport.name}: QA route returned ${response?.status() ?? "no response"}`);
      await page.waitForSelector("[data-akkari-visual-qa-grid] [data-akkari-qa-card]", { timeout: 15_000 });
      ...
      if (metrics.cards.length !== 3) throw new Error(`${viewport.name}: expected 3 Akkari cards, got ${metrics.cards.length}`);
      ...
      for (const card of metrics.cards) {
        ...
        if (card.title !== "AKKARI" || card.city !== "Rabat") throw new Error(`${viewport.name}/${card.role}: identity drift`);
        ...
        if (card.role === "signature" && !card.credit.includes("RACHID BAYA")) throw new Error(`${viewport.name}: signature author credit missing`);
        if (card.role !== "signature" && !card.credit.includes("KartaView Contributors")) throw new Error(`${viewport.name}/${card.role}: KartaView credit missing`);
      ...
      results.push({ viewport: viewport.name, scene_roles: roles, visual_ids: ids, decoded_sources: decodedSources, card_widths: metrics.cards.map((card) => card.width), visible_details_per_card: viewport.detailCount, horizontal_overflow: false, duplicate_labels: false, disclosure: "Photo d’ambiance", attribution: "source-aware", machine_quality_score: 10 });
