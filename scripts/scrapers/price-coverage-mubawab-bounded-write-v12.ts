...
const SOURCE = "mubawab.ma";
const WRITE_CONFIRMATION = "WRITE_100_MUBAWAB_RELIABLE_PRICES";
const MAX_PAGES = 5;
...
export function hasExplicitMubawabWriteConfirmation(raw: string | undefined) {
...
async function writeOne(row: SnapshotRow, amount: number) {
  ...
  if (count !== 1) throw new Error(`write cardinality violation for ${row.seed_id}: expected 1 row, got ${count}`);
  return count;
}

async function main() {
  const guard = getThirdPartyIngestionGuard({ scriptName: "price-coverage-mubawab-bounded-write-v12" });
  if (guard.blocked) throw new Error(guard.message);

  const write = process.env.PRICE_COVERAGE_V12_WRITE === "true";
  if (write && !hasExplicitMubawabWriteConfirmation(process.env.PRICE_COVERAGE_V12_WRITE_CONFIRMATION)) {
    throw new Error("price coverage v12 write blocked: explicit Mubawab confirmation missing");
  }

  const pageSize = Math.max(20, Math.min(Number(process.env.PRICE_COVERAGE_V12_PAGE_SIZE ?? 120), 200));
  const pages = Math.max(1, Math.min(Number(process.env.PRICE_COVERAGE_V12_PAGES ?? MAX_PAGES), MAX_PAGES));
  const maxWrites = Math.max(1, Math.min(Number(process.env.PRICE_COVERAGE_V12_MAX_WRITES ?? MAX_WRITES), MAX_WRITES));
  ...
  for (const row of snapshot) {
    ...
    try {
      ...
      stats.reliable += 1;
      if (write) stats.written += await writeOne(row, audit.amount);
    } catch (error) {
      stats.failed += 1;
