...
import type { ExistingSourceOfferSeedIdentity } from "./external-index-seed-write-plan";
...
async function fetchExistingSeeds(
...
): Promise<ExistingSourceOfferSeedIdentity[]> {
  ...
  for (let index = 0; index < canonicalUrls.length; index += LOOKUP_CHUNK) {
    ...
    if (error) throw error;
    rows.push(...((data ?? []) as ExistingSourceOfferSeedIdentity[]));
  }
  return rows;
}
...
async function main() {
  ...
  const validUrls = manifest
    .filter((row) => row.promotionStatus === "EXTERNAL_INDEX_CANDIDATE" && row.canonicalUrl)
  ...
  const result = {
    schemaVersion: "MASS_INDEX_M4_NATIONAL_PLAN_V1",
    mode: "read_only",
