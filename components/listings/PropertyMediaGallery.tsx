  ...
  return (
    <>
      <button
        type="button"
        onClick={previous}
        className="absolute left-3 top-1/2 z-20 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur hover:bg-black/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
      ...
      <button
        type="button"
        onClick={next}
        className="absolute right-3 top-1/2 z-20 grid h-11 w-11 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur hover:bg-black/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
      ...
      <button
        type="button"
        onClick={open}
        className="absolute bottom-4 right-4 z-20 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-3 text-[11px] font-extrabold text-[#0B2545] shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE]"
...
export function PropertyMediaGallery({
  ...
  const fullscreenDialog = fullscreen && active ? (
    <div
      ref={dialogRef}
      role="dialog"
      aria-modal="true"
      aria-label={`Galerie photos de ${listing.title}`}
      className="fixed inset-0 z-[100] flex flex-col bg-[#061425]/95 text-white backdrop-blur-sm"
    >
        ...
        {items.length > 1 ? (
          <>
            <button
              type="button"
              onClick={previous}
              className="absolute left-3 grid h-12 w-12 place-items-center rounded-full bg-black/45 hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:left-5"
              aria-label="Photo précédente"
            >
              <ChevronLeft size={26} />
            </button>
            <button
              type="button"
              onClick={next}
              className="absolute right-3 grid h-12 w-12 place-items-center rounded-full bg-black/45 hover:bg-black/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:right-5"
              aria-label="Photo suivante"
            >
      ...
      {model.attribution ? (
        <p className="px-4 pb-4 text-center text-[11px] text-white/65">{model.attribution}</p>
      ) : null}
    </div>
  ) : null;
        ...
        >
          <div className="relative h-[320px] sm:h-[500px]">
            ...
            <button
              ...
              className="absolute bottom-4 right-4 z-20 inline-flex min-h-11 items-center gap-2 rounded-xl bg-white px-3 text-[11px] font-extrabold text-[#0B2545] shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B63CE] lg:px-4 lg:text-[12px]"
              aria-label="Agrandir la photo"
            >
              <Maximize2 size={17} /> <span className="hidden lg:inline">Agrandir</span>
            </button>
          </div>
        </section>
        {fullscreenDialog}
      ...
      >
        ...
        <div className="relative hidden h-[500px] gap-1.5 bg-slate-100 lg:grid lg:grid-cols-[1.55fr_1fr]">
          <button
            ...
            className="relative overflow-hidden text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B63CE]"
          ...
          <div className="grid min-w-0 grid-rows-2 gap-1.5">
            {[items[1], items[2]].map((item, index) => item ? (
              <button
                ...
                className="relative overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B63CE]"
