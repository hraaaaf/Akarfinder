...
const outputDir = `data/audits/neighborhood-visual-p1-4-hassan/${variant}`;
const viewports = [
...
const expectedIds = ["rabat-hassan-signature-v1", "rabat-hassan-immobilier-v1", "rabat-hassan-lifestyle-v1"];
const expectedSourceDimensions = [
  { path: "/__qa/hassan-signature.jpg", width: 5184, height: 3456 },
  { path: "/__qa/hassan-immobilier.jpg", width: 3456, height: 2304 },
  { path: "/__qa/hassan-lifestyle.jpg", width: 1375, height: 2048 },
];
...
async function readMetrics(page) {
  return page.evaluate(() => {
    ...
    const cards = [...document.querySelectorAll("[data-hassan-qa-card]")];
    return {
      cards: cards.map((card) => {
        ...
        return {
          ...
          credit: text(card.querySelector("[data-hassan-qa-credit]")),
...
try {
  for (const viewport of viewports) {
    ...
    try {
      const response = await page.goto(`${baseUrl}/visual-qa/hassan`, { waitUntil: "networkidle", timeout: 45_000 });
      if (!response || response.status() >= 400) throw new Error(`${viewport.name}: QA route returned ${response?.status() ?? "no response"}`);
      await page.waitForSelector("[data-hassan-visual-qa-grid] [data-hassan-qa-card]", { timeout: 15_000 });
      ...
      if (metrics.cards.length !== 3) throw new Error(`${viewport.name}: expected 3 Hassan cards, got ${metrics.cards.length}`);
      ...
      for (const card of metrics.cards) {
        ...
        if (card.title !== "HASSAN" || card.city !== "Rabat") throw new Error(`${viewport.name}/${card.role}: identity drift`);
        ...
        if (!card.credit.includes("CC BY-SA")) throw new Error(`${viewport.name}/${card.role}: license credit missing`);
