...
import { AKKARI_NEIGHBORHOOD_VISUALS } from "@/lib/contextual-illustrations/akkari-neighborhood-visuals";
import type { Listing } from "@/lib/listings/types";

const LOCAL_QA_ASSETS: Record<(typeof AKKARI_NEIGHBORHOOD_VISUALS)[number]["sceneRole"], string> = {
  signature: "/__qa/akkari-signature.jpg",
  immobilier: "/__qa/akkari-immobilier.jpg",
  lifestyle: "/__qa/akkari-lifestyle.jpg",
};

function qaListing(index: number, sceneRole: (typeof AKKARI_NEIGHBORHOOD_VISUALS)[number]["sceneRole"]): Listing {
  return {
    id: `akkari-visual-qa-${sceneRole}`,
    title: "Bien à Akkari — aperçu de certification",
    city: "Rabat",
    neighborhood: "Akkari",
    price: 1_420_000 + index * 120_000,
    currency: "DH",
    surface_m2: 92 + index * 10,
    price_per_m2: null,
    ...
    listing_url: `https://example.com/qa/akkari/${sceneRole}`,
...
function creditLabel(visual: (typeof AKKARI_NEIGHBORHOOD_VISUALS)[number]): string {
  return `${visual.source.author} · ${visual.source.license} · ${visual.source.sourceName}`;
}

export function AkkariNeighborhoodVisualQAFixture() {
  return (
    <PropertySelectionProvider>
      <section data-akkari-visual-qa-grid className="grid grid-cols-2 gap-x-2.5 gap-y-6 lg:grid-cols-4 lg:gap-x-3">
        {AKKARI_NEIGHBORHOOD_VISUALS.map((visual, index) => {
          const qaSource = LOCAL_QA_ASSETS[visual.sceneRole];
          const style = { "--akkari-qa-source": `url(\"${qaSource}\")` } as CSSProperties;
      ...
      <style jsx global>{`
        [data-akkari-qa-card] [data-card-image] > div:first-child { opacity: 0 !important; }
        [data-akkari-qa-card] [data-card-image]::before {
          content: "";
          position: absolute;
          inset: 0;
          z-index: 0;
          background-image: var(--akkari-qa-source);
          background-position: center;
          background-repeat: no-repeat;
          background-size: cover;
        }
        [data-akkari-qa-card] [data-card-image] > div:nth-child(3),
        [data-akkari-qa-card] [data-card-image] > span:not([data-neighborhood-photo-title]):not([data-neighborhood-photo-disclosure]) { z-index: 20; }
      `}</style>
    </PropertySelectionProvider>
  );
}
