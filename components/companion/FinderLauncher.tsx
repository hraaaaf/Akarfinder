  ...
  return (
    <>
      ...
      {open ? <div className="fixed inset-0 z-[100]" role="presentation">
        <button type="button" aria-label="Fermer Akar Sense" onClick={() => setOpen(false)} className="absolute inset-0 hidden bg-[#071B33]/30 backdrop-blur-[2px] sm:block" />
        <section role="dialog" aria-modal="true" aria-label="Akar Sense, assistant de projet immobilier" className="absolute inset-0 flex h-[100dvh] w-full flex-col bg-[#F4F8FC] sm:inset-y-0 sm:left-auto sm:right-0 sm:w-[min(520px,94vw)] sm:shadow-[-24px_0_70px_rgba(7,27,51,0.18)]" data-finder-panel>
          <header className="flex min-h-16 shrink-0 items-center justify-between border-b border-[#DCE8F5] bg-white px-4 sm:px-5">
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label="Fermer Akar Sense" className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 text-slate-600 hover:bg-slate-50"><X size={18} /></button>
          </header>
