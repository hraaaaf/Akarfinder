...
import { ArrowRight, Building2, FileSearch, MapPin, X } from "lucide-react";
import { PropertyPassportPanel } from "@/components/search/PropertyPassportPanel";
...
export function PropertyQuickPreview() {
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1480px] rounded-2xl border border-bronze-500/30 bg-card p-4 shadow-[0_16px_42px_rgba(2,10,24,0.16)] dark:bg-white/[0.045] sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => setPassportOpen((current) => !current)}
              aria-expanded={passportOpen}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-bronze-500/30 bg-bronze-500/10 px-4 py-3 text-[12.5px] font-extrabold text-bronze-300 transition hover:bg-bronze-500/20 sm:flex-none"
            >
              <FileSearch size={14} aria-hidden="true" />
              {passportOpen ? "Masquer le passeport" : "Property Passport"}
            </button>
            <Link
              href={href}
        ...
        {passportOpen ? <PropertyPassportPanel listing={activeListing} onClose={() => setPassportOpen(false)} /> : null}
      </div>
    </section>
  );
}
