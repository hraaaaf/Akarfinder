...
**Statut : ACTIVE — N3 NATIONAL PROUVÉ / UX L9 PROUVÉ / INTÉGRATION CURRENT MAIN PROUVÉE / LIVE DATA BLOQUÉ PAR SUPABASE / HUMAN MERGE GATE**  
**Dernière mise à jour : 2026-09-17**  
**Repo : `hraaaaf/Akarfinder`**  
**Branche active : `integration/vivre-ici-main-2026-09-17`**  
**PR d’intégration : #1037 DRAFT / HUMAN MERGE GATE**  
**PR historique : #1025 OPEN / non utilisée pour le merge direct**  
**HEAD produit+harness UX L9 source : `f1f4d35ecdd0a5a6b83df1dc945e2d291e9b2533`**  
**HEAD intégration avant closeout docs : `33e30f72b6c26e02a60f460d6fad73856c717599`**  
...
### État du Goal
**Interne/UI : prouvé. Intégration sur le `main` courant : prouvée sur branche dédiée. Live data : non certifiable actuellement à cause du quota Supabase. Merge : non autorisé, arrêté au human gate.**
...
## 8.1 INTÉGRATION CURRENT MAIN — 2026-09-17

Base vérifiée : `main@8578f7a492980dcac35e7a094383c70411ca44c5`.

Stratégie exécutée :
- branche dédiée `integration/vivre-ici-main-2026-09-17` ;
- transplant ciblé du produit Vivre Ici, sans rebase sauvage de la branche longue ;
- exactement 5 fichiers shell/navigation en intersection avec le nouveau `main` conservés selon le contrat courant ;
- 64 chemins produit/harness intégrés ;
- aucun Vercel ; aucun write DB.

Commits :
- `0c84954d1c2ce53938677e9cb2b89e9241c52260` — transplant produit sur current main ;
- `f5d5bce0edac47021953cd7cb091fc60fde52cdd` — HEAD CI temporaire pour déclencher les deux gates ;
- `33e30f72b6c26e02a60f460d6fad73856c717599` — triggers restaurés ; arbre produit final restauré à `b04bc649746b5b2a3733e058af0ca59df8dc2930`, identique à celui du transplant.

Preuves intégration :
- AFTER run `35282869300` ✅ SUCCESS ;
- artifact AFTER `10523630593`, digest `sha256:c4d0fe087d7194bb5554c109f87d5d9750ffd088d9f2edf94a4a0bc15822a4fb` ;
- N3 run `35282869261` ✅ SUCCESS ;
- artifact N3 `10523455762`, digest `sha256:700a62489b3943345ca505829e11e92801a4a0420cf232d9fe9a7e351432eb4b`.

AFTER intégré — matrice `390x844 / 430x932 / 768x900 / 1280x900`, national + N3 :
- HTTP 200 sur 8/8 ;
- overflow horizontal 0 sur 8/8 ;
- Supabase requests 0 ;
- page errors 0 ;
- national : 12 régions, TopoJSON ready ;
- N3 : MapLibre ready, OpenFreeMap vector, bâtiments > 0 ;
- `anchor_count=0` → faux signal grid masqué ;
- retour Maroc et CTA Search exacts ;
- `zeroDbWritesByScript=true` ;
- `zeroDeploymentActionsByScript=true`.

Comparaison visuelle avec BEFORE run `34977355451` / artifact `10399777496` :
- territoire national reste nettement agrandi sur les 4 viewports ;
- N3 mobile reste map-first et les faux indicateurs zéro restent supprimés ;
- le shell/navigation courant de `main` est préservé, notamment `/map → Vivre ici` et `Vendre`.
- score maintenu à **9,2/10** ; aucun `>=9,8` revendiqué.

PR d’intégration : **#1037 DRAFT**. Aucun merge autorisé à ce stade.
...
- [x] résoudre l’intégration Vivre Ici avec le `main` courant en préservant le nouveau shell/navigation
- [x] rerun AFTER/N3 sur le produit intégré et inspecter les captures mêmes viewports
...
1. Garder PR #1037 en DRAFT et **ne pas merger sans autorisation humaine explicite**.
2. Conserver les gates live fail-closed tant que Supabase reste restreint par `exceed_egress_quota`.
3. Dès restauration Supabase : rerun uniquement les workflows live impactés.
4. Si merge #1037 est explicitement autorisé : merge → post-merge checks → mise à jour canonique.
5. Lot sécurité Next/npm et décision provider/licence/attribution restent ouverts.
6. Vercel uniquement sur autorisation explicite.
