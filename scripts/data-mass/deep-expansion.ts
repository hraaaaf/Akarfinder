...
export type DeepExpansionPattern = "DABA_HEX_DETAIL" | "MARRAKECHREALTY_STATUS_SLUG" | "YAKEEY_ID_SUFFIX" | "SOUQCITY_LISTING_ID" | "SOUQCITY_AD_ID" | "JIBRIL_BIENS_SLUG" | "SW_PROPRIETE_SLUG" | "ATLAS_PROPERTY_SLUG" | "LOCO_IMMOBILIERS_SLUG";

function normalizedDomain(value: string): string { return value.trim().toLowerCase().replace(/^www\./, ""); }
function pathname(rawUrl: string): string { try { return new URL(rawUrl).pathname.toLowerCase().replace(/\/+$/, "") || "/"; } catch { return rawUrl.toLowerCase().split("?")[0]?.replace(/\/+$/, "") || rawUrl.toLowerCase(); } }
...
export type DeepExpansionResult = CandidateClassification & { deepExpansionPattern: DeepExpansionPattern | null; upgradedByMassX2: boolean };
export function applyDeepExpansion(candidate: ReservoirCandidate, base: CandidateClassification): DeepExpansionResult {
  const pattern = detectDeepExpansionPattern(candidate);
  const canUpgrade = Boolean(pattern && base.likelyRealEstate && base.geographyScope === "MOROCCO_LIKELY" && base.pageKind === "AMBIGUOUS");
  if (!canUpgrade) return { ...base, deepExpansionPattern: pattern, upgradedByMassX2: false };
  return { ...base, pageKind: "LIKELY_LISTING_DETAIL", reasons: [...base.reasons, `MASS_X2_${pattern}`], deepExpansionPattern: pattern, upgradedByMassX2: true };
}
