...
import { isExternalIndexSafeCanonicalUrl } from "./external-index-seed";
...
export function filterM4Wave1Manifest(
  ...
  return rows
    ...
    .filter((row) => isExternalIndexSafeCanonicalUrl(row.canonicalUrl!))
...
export function summarizeM4Plan(
  ...
  const byDomain = M4_WAVE1_DOMAINS.map((sourceDomain) => {
    ...
    const sourceSpecificValid = sourceSpecificDetail.filter((row) => isExternalIndexSafeCanonicalUrl(row.canonicalUrl!));
