...
import {
  normalizeStableRepresentationKey,
...
describe("CONTEXTUAL-ILLUSTRATIONS-FOUNDATION-1", () => {
  ...
  it("normalizes only representation identity noise and keeps meaningful query state", () => {
    assert.equal(
      normalizeStableRepresentationKey(
        "https://Example.com/property/123/?utm_source=google&GCLID=abc&b=2&a=1#gallery"
      ),
      "example.com/property/123?a=1&b=2"
    );
    assert.equal(
      normalizeStableRepresentationKey("https://example.com/property/123?a=1&b=2"),
      "example.com/property/123?a=1&b=2"
    );
    assert.notEqual(
      normalizeStableRepresentationKey("https://example.com/property/123?unit=1"),
      normalizeStableRepresentationKey("https://example.com/property/123?unit=2")
    );
    assert.equal(normalizeStableRepresentationKey("not a url"), "");
  });

  it("keeps the same asset when only tracking parameters or URL presentation change", () => {
    const catalog: ContextualIllustrationCatalog = {
      districtType: {},
      district: {},
      cityType: {},
      city: { Agadir: syntheticAssets },
    };
    const clean = resolveContextualIllustrationFromCatalog(
      {
        stableRepresentationKey: "https://example.com/property/stable?a=1&b=2",
        normalizedCity: "Agadir",
      },
      catalog
    );
    const tracked = resolveContextualIllustrationFromCatalog(
      {
        stableRepresentationKey:
          "https://EXAMPLE.com/property/stable/?utm_campaign=x&b=2&a=1&fbclid=tracking#photos",
        normalizedCity: "Agadir",
      },
      catalog
    );
    assert.deepEqual(tracked, clean);
  });
  ...
  it("uses the most specific certified structured pool and fails closed by tier", () => {
    ...
    assert.equal(
      resolveContextualIllustrationFromCatalog(
        { stableRepresentationKey: "not a url", normalizedCity: "Agadir" },
        catalog
      ),
      null,
      "malformed representation identity must fail closed"
    );
  });
