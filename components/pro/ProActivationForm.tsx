...
export function ProActivationForm() {
  ...
  const fieldClass = "mt-2 w-full rounded-xl border border-white/15 bg-white/[0.07] px-4 py-3 text-sm normal-case tracking-normal text-white outline-none transition focus:border-[#3B82F6] focus:ring-2 focus:ring-[#0B63CE]/20";

  return (
    <section id="contact" className="bg-[#061027] py-16 lg:py-20" data-p9-activation>
      <Container>
        <div className="mx-auto max-w-2xl">
          <div className="text-center">
            <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-blue-300">Demande d’activation</p>
          ...
          {result?.ok ? (
            ...
            <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.055] p-6 sm:p-8">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Nom<input value={form.name} onChange={(e) => set("name", e.target.value)} className={fieldClass} /></label>
                <label className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Société *<input value={form.company} onChange={(e) => set("company", e.target.value)} className={fieldClass} /></label>
                <label className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Téléphone *<input inputMode="tel" autoComplete="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} className={fieldClass} /></label>
                <label className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Profil *<select value={form.type} onChange={(e) => set("type", e.target.value)} className={`${fieldClass} bg-[#102744]`}><option value="">Sélectionner…</option>{TYPES.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select></label>
                <label className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55 sm:col-span-2">Ville<input value={form.city} onChange={(e) => set("city", e.target.value)} className={fieldClass} /></label>
              </div>

              <fieldset className="mt-5">
                <legend className="text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Options à discuter</legend>
                <div className="mt-3 grid gap-2">
                  {ADDONS.map((addon) => <label key={addon.value} className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-[12.5px] text-white/75 transition hover:border-[#0B63CE]/45"><input type="checkbox" checked={form.addons.includes(addon.value)} onChange={() => toggleAddon(addon.value)} />{addon.label}</label>)}
                </div>
              </fieldset>

              <label className="mt-5 block text-[11px] font-extrabold uppercase tracking-[0.08em] text-white/55">Message<textarea rows={3} value={form.message} onChange={(e) => set("message", e.target.value)} className={fieldClass} /></label>

              <label className="mt-5 flex cursor-pointer items-start gap-3 rounded-xl border border-white/10 bg-white/[0.04] p-4 text-[12.5px] leading-5 text-white/70 transition hover:border-[#0B63CE]/45"><input type="checkbox" checked={form.consent} onChange={(e) => set("consent", e.target.checked)} className="mt-0.5" /><span>J’accepte d’être recontacté au sujet de l’activation Pro. Je comprends qu’une demande ne crée aucun statut partenaire, badge, publication ou résultat garanti.</span></label>

              {result && !result.ok ? <p className="mt-4 rounded-xl bg-red-500/15 p-4 text-sm font-semibold text-red-200">{result.error}</p> : null}
              <button type="button" disabled={!canSubmit} onClick={submit} className="mt-5 w-full rounded-xl bg-[#0B63CE] px-6 py-3.5 text-[14px] font-extrabold text-white transition hover:bg-[#0957B7] disabled:opacity-45">{busy ? "Enregistrement…" : "Envoyer ma demande d’activation"}</button>
            </div>
          )}
        </div>
      </Container>
