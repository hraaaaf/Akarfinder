...
function FilterFields({ city, propertyType }: { city?: string; propertyType?: string }) {
  return (
    <>
      <label className="grid gap-2 text-sm font-extrabold">Localisation<input name="city" defaultValue={city ?? ""} placeholder="Casablanca" className={field} /></label>
      ...
      <label className="grid gap-2 text-sm font-extrabold">Type de bien<select name="property_type" defaultValue={propertyType ?? ""} className={field}><option value="">Tous les types</option><option value="Appartement">Appartement</option><option value="Villa">Villa</option><option value="Maison">Maison</option><option value="Terrain">Terrain</option><option value="Studio">Studio</option><option value="Bureau">Bureau</option></select></label>
...
export function SearchFiltersV2({ total, city, propertyType }: { total: number; city?: string; propertyType?: string }) {
  const [open,setOpen] = useState(false);
  return (
    <>
      <aside className="hidden border-r border-slate-200 bg-white p-5 lg:block">
        <form action="/search" className="space-y-5"><div className="flex items-center justify-between"><h2 className="text-lg font-black">Filtres</h2><a href="/search" className="text-xs font-extrabold text-blue-700">Réinitialiser</a></div><FilterFields city={city} propertyType={propertyType}/><button className="h-11 w-full rounded-xl bg-blue-700 text-sm font-extrabold text-white">Voir {total} résultats</button></form>
      </aside>
