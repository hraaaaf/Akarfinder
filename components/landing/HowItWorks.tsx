...
export function HowItWorks() {
  return (
    <section className="bg-[#f9fafb] py-14 sm:py-18">
      <Container>
        <div className="max-w-2xl">
          <p className="text-[12px] font-bold uppercase tracking-[0.18em] text-[#2563eb]">Comment ça marche</p>
          <h2 className="mt-3 text-3xl font-bold leading-tight tracking-[-0.03em] text-gray-900 sm:text-[2.4rem]">
        ...
        <div className="mt-9 grid gap-5 md:grid-cols-3">
          {steps.map((s) => (
            <div key={s.n} className="group relative rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_2px_14px_rgba(0,0,0,0.05)] transition duration-300 hover:-translate-y-1.5 hover:shadow-[0_18px_40px_rgba(15,40,80,0.12)]">
              ...
              <h3 className="mt-4 text-[1.15rem] font-bold tracking-tight text-gray-900">{s.title}</h3>
              <p className="mt-2 text-[14px] leading-7 text-gray-600">{s.body}</p>
            </div>
          ))}
        </div>
      </Container>
