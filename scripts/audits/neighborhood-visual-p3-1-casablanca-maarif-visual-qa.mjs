...
try {
  for (const [name, width, height] of viewports) {
    ...
    try {
      const response = await page.goto(`${baseUrl}/visual-qa/casablanca-maarif-p3`, { waitUntil: "networkidle", timeout: 45000 });
      ...
      if (new Set(metrics.cards.map((card) => card.resolved)).size !== 3) throw new Error(`${name}: resolver did not expose three distinct Maârif masters`);
      for (const card of metrics.cards) {
        ...
        if (!/^Crédit & licence · (Wikimedia Commons|KartaView)$/.test(card.credit)) throw new Error(`${name}: source credit drift ${card.credit}`);
      ...
      results.push({ viewport: name, machine_quality_score: 10, horizontal_overflow: false, real_photo_cards: 3, distinct_masters: 3, truth_safe_labels: true, exact_source_credits: true, images_loaded: 3 });
    } catch (error) {
      failure = error;
      break;
    } finally {
