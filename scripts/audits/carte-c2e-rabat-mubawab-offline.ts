...
const OUTPUT = join(process.cwd(), "data/audits/runtime/carte-c2e-rabat-mubawab-offline.json");
...
async function readByIds(db: any, table: string, select: string, key: string, ids: string[]): Promise<any[]> {
  ...
  for (const batch of chunks(ids)) {
    ...
    if (error) throw new Error(`C2E ${table} bounded read failed: ${err(error)}`);
    rows.push(...(data ?? []));
  }
  return rows;
}
...
async function main() {
  ...
  const { data: cityRows, error: cityError } = await db
    ...
    .limit(2);
  if (cityError) throw new Error(`C2E Rabat city read failed: ${err(cityError)}`);
  if ((cityRows ?? []).length !== 1) throw new Error(`C2E expected one validated Rabat city, got ${(cityRows ?? []).length}`);
  ...
  const { data: neighborhoodRows, error: neighborhoodError } = await db
    ...
    .eq("validation_status", "validated")
    .in("slug", [...TARGETS]);
  if (neighborhoodError) throw new Error(`C2E neighborhoods read failed: ${err(neighborhoodError)}`);
  const neighborhoodById = new Map((neighborhoodRows ?? []).map((row: any) => [String(row.id), String(row.slug)]));
  const found = new Set(neighborhoodById.values());
  for (const slug of TARGETS) if (!found.has(slug)) throw new Error(`C2E missing validated neighborhood ${slug}`);
  ...
  const { data: targetEvents, error: targetEventsError } = await db
    ...
    .range(0, 999);
  if (targetEventsError) throw new Error(`C2E target events read failed: ${err(targetEventsError)}`);
  if ((targetEvents ?? []).length >= 1000) throw new Error("C2E target event safety bound reached");

  const candidateSeedIds = [...new Set((targetEvents ?? []).map((r: any) => String(r.source_record_id)).filter(Boolean))];
  const allEvents = await readByIds(db, "geo_resolution_events", "id,source_record_type,source_record_id,resolution_status,resolved_city_id,resolved_neighborhood_id,created_at", "source_record_id", candidateSeedIds);
  ...
  const report = {
    contractVersion: "carte_c2e_rabat_mubawab_offline_v1",
    readOnly: true,
    thirdPartyFetches: 0,
    source: "mubawab_v4_stored_evidence_only",
    targetSlugs: TARGETS,
    candidateCount: rows.length,
    matchedCount: matched.length,
    byZone,
    results,
    verdict: matched.length >= 2 ? "C2E_OFFLINE_RECOVERY_REVIEWABLE" : "C2E_OFFLINE_RECOVERY_INSUFFICIENT",
  };
