-- ODM Public Listing-Only Gate V1
-- Responsibility: prevent CATEGORY / AMBIGUOUS documents from reaching the public ODM result set.
-- Legacy serving, Canary percentage and fallback behavior are unchanged.
...
select representation_id, canonical_url, source_domain, seed_provider, freshness_status, title, snippet,
  normalized_city, normalized_property_type, normalized_intent, normalized_price_mad, normalized_surface_m2,
  price_per_m2_mad, quality_tier, quality_score, display_eligibility, display_eligibility_reason,
  ranking_quality_boost, updated_at, lane_weight, ranking_score, total_count
from page;
$function$;

comment on function public.search_public_representations_v1(text,text,text,text,numeric,numeric,numeric,numeric,integer,smallint,real,timestamptz,uuid)
is 'ODM public search V1. Returns display-eligible LISTING documents only; CATEGORY and AMBIGUOUS documents are excluded.';
