...
const expectedHrefs = ["/mon-projet", "/vendre", "/pro"];
const forbiddenCopy = ["Votre recherche, simplement", "Préparez votre projet au Maroc, où que vous soyez.", "Vous savez déjà ce que vous cherchez ?", "4 000 000 DH", "Biens enregistrés", "Rechercher un bien"];
...
try {
  for (const scenario of scenarios) {
    ...
    try {
      ...
      const grid = page.locator('[data-home-action-grid="v1"]');
      await grid.waitFor({ state: "visible", timeout: 20_000 });
      const cards = grid.locator("[data-home-action]");
      ...
      if (heading !== "La suite de votre projet") localFindings.push(`HEADING_${heading}`);
      ...
      results.push({ ...scenario, screenshot, heading, cardCount, hrefs, consoleErrors, findings: localFindings, ...metrics });
    } catch (error) {
...
const report = { schemaVersion: "HVR_5_HOME_V1_SIMPLIFICATION_PROOF", generatedAt: new Date().toISOString(), scenarioCount: scenarios.length, screenshotCount: results.filter((item) => item.screenshot).length, findingCount: findings.length, findings, results };
