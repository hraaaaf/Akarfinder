...
async function latestIndexes(){const r=await fetch('https://index.commoncrawl.org/collinfo.json');if(!r.ok)throw new Error(`collinfo ${r.status}`);return (await r.json()).slice(0,3).map(x=>x.id)}
async function findRecord(url,indexes){for(const idx of indexes){const q=`https://index.commoncrawl.org/${idx}-index?url=${encodeURIComponent(url)}&output=json&filter=status:200&filter=mime:text/html&collapse=urlkey`;const r=await fetch(q);if(!r.ok)continue;const text=await r.text();const lines=text.split('\n').filter(Boolean);if(lines.length){try{return JSON.parse(lines[0])}catch{}}}return null}
async function fetchHtml(rec){const start=Number(rec.offset),len=Number(rec.length);const r=await fetch(`https://data.commoncrawl.org/${rec.filename}`,{headers:{Range:`bytes=${start}-${start+len-1}`}});if(!r.ok)throw new Error(`warc ${r.status}`);const buf=Buffer.from(await r.arrayBuffer());const dec=zlib.gunzipSync(buf).toString('utf8');const p=dec.indexOf('\r\n\r\n');const rest=p>=0?dec.slice(p+4):dec;const p2=rest.indexOf('\r\n\r\n');return p2>=0?rest.slice(p2+4):rest}

const indexes=await latestIndexes();const recovered=[];let indexMatches=0,warcFetched=0,errors=0
for(let i=0;i<targets.length;i+=10){const batch=targets.slice(i,i+10);const rows=await Promise.all(batch.map(async t=>{try{const rec=await findRecord(t.canonical_url,indexes);if(!rec)return{...t,index_match:false};indexMatches++;const html=await fetchHtml(rec);warcFetched++;const e=extract(html);const price=t.price_mad||e.price;const surface=t.surface_m2||e.surface;return{...t,index_match:true,recovered_price:e.price,recovered_surface:e.surface,priceCandidates:e.priceCandidates,surfaceCandidates:e.surfaceCandidates,final_price:price,final_surface:surface,complete:Boolean(price>0&&surface>0),cc_timestamp:rec.timestamp,cc_filename:rec.filename}}catch(err){errors++;return{...t,error:String(err?.message||err)}}}));recovered.push(...rows)}
