...
const seedProjector = readFileSync("scripts/data-mass/external-index-seed.ts", "utf8");
...
test("M4 filters sensitive canonical URLs and sanitizes external text before planning writes", () => {
  assert.match(planner, /isExternalIndexSafeCanonicalUrl/);
  assert.match(seedProjector, /redactSensitiveText/);
  assert.match(seedProjector, /EXTERNAL_INDEX_SNIPPET_MAX_CHARS\s*=\s*320/);
  assert.match(seedProjector, /MASS_INDEX_M2_SENSITIVE_CANONICAL_URL/);
});
