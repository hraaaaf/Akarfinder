...
const commonsSource = (d: { fileName: string; asset: string; sourcePage: string; author: string; license: string; rightsBasis: "cc_by_3_0" | "cc_by_sa_2_0"; rightsNote: string; width: number; height: number; bytes: number; sha1: string; locationEvidence: string }) => ({
  fileName: d.fileName,
  asset: d.asset,
...
export const MAARIF_SIGNATURE_VISUAL = {
  ...
  truthBoundary: { depictsSpecificProperty: false, claimInsideNeighborhood: false, claimPropertyForSale: false, allowedMeaning: "Urban Maârif context only; never a listing-specific property claim." },
} as const;
...
export const MAARIF_IMMOBILIER_VISUAL = {
  ...
  truthBoundary: { depictsSpecificProperty: false, claimInsideNeighborhood: false, claimPropertyForSale: false, allowedMeaning: "Residential street morphology near the Maârif discovery area only." },
} as const;
...
export const MAARIF_LIFESTYLE_VISUAL = {
  ...
  truthBoundary: { depictsSpecificProperty: false, claimInsideNeighborhood: false, claimPrivateAmenity: false, claimPropertyForSale: false, allowedMeaning: "Public-space lifestyle context explicitly associated with Maârif only." },
} as const;

export const MAARIF_NEIGHBORHOOD_VISUALS = [MAARIF_SIGNATURE_VISUAL, MAARIF_IMMOBILIER_VISUAL, MAARIF_LIFESTYLE_VISUAL] as const;
