...
export function MapLibreMaarifSpike() {
  ...
  useEffect(() => {
    ...
    void import("maplibre-gl")
      .then((maplibregl) => {
        ...
        map.once("load", () => {
          ...
          try {
            ...
            if (maarifGeometry?.geometry) {
              map.addSource("maarif-boundary", {
                type: "geojson",
                data: { type: "Feature", properties: {}, geometry: maarifGeometry.geometry } as any,
              });
              map.addLayer({
                id: "maarif-boundary-fill", type: "fill", source: "maarif-boundary",
                paint: { "fill-color": "#0b6668", "fill-opacity": 0.025 },
              });
              map.addLayer({
                id: "maarif-boundary-line", type: "line", source: "maarif-boundary",
                paint: { "line-color": "#f4ddaf", "line-width": 1.4, "line-opacity": 0.55 },
              });
            }
          } catch (error) {
            console.error("[vivre-ici-maplibre-spike] layer setup failed", error);
      ...
      .catch((error) => {
        console.error("[vivre-ici-maplibre-spike] renderer failed", error);
    ...
    >
      <div className="maplibre-spike-map" data-maplibre-map-surface>
        ...
        <div className="maplibre-spike-dom-labels" aria-hidden="true">
          {centerPoint?.visible && <div className="maplibre-spike-neighborhood-label" style={{ left: centerPoint.x, top: centerPoint.y }}>Maârif</div>}
      ...
      <div className="maplibre-spike-map-chrome">
        <div className="maplibre-spike-location">←&nbsp; Vivre à Casablanca</div>
      ...
      <div className="maplibre-spike-filters" aria-label="Filtres des repères du quartier">
      ...
      <div className="maplibre-spike-map-note"><strong>Maârif · MapLibre national spike</strong><span>{buildingCount > 0 ? `${buildingCount} volumes visibles` : "Chargement des volumes…"}</span></div>
      ...
      <style jsx global>{`
        .maplibre-spike-shell{position:relative;display:grid;grid-template-columns:minmax(0,1fr) 372px;grid-template-rows:minmax(0,1fr) 76px;gap:8px;height:calc(100svh - 64px);padding:10px 10px 0 12px;background:#f4efe7;color:#17302e;overflow:hidden}
        ...
        .maplibre-spike-map-chrome{position:absolute;z-index:12;left:28px;right:398px;top:28px;display:grid;grid-template-columns:auto minmax(220px,1fr) auto;gap:10px}.maplibre-spike-location,.maplibre-spike-search,.maplibre-spike-mode{min-height:40px;display:flex;align-items:center;border:1px solid rgb(255 255 255/.88);background:rgb(255 255 255/.94);box-shadow:0 8px 26px rgb(30 46 47/.12);font-size:10px;font-weight:800;color:#243a39}.maplibre-spike-location{padding:0 13px;border-radius:999px}.maplibre-spike-search{padding:0 16px;border-radius:999px;color:#7b817e}.maplibre-spike-mode{padding:3px;border-radius:999px}.maplibre-spike-mode span,.maplibre-spike-mode strong{display:grid;place-items:center;min-width:37px;min-height:32px;border-radius:999px}.maplibre-spike-mode strong{background:#0b6668;color:#fff}
        .maplibre-spike-filters{position:absolute;z-index:13;left:190px;right:404px;top:78px;display:flex;gap:7px;overflow:hidden}.maplibre-spike-filters button{flex:0 0 auto;border:0;padding:7px 11px;border-radius:999px;background:rgb(255 255 255/.94);box-shadow:0 6px 18px rgb(30 46 47/.10);font-size:8px;font-weight:800;color:#364845}.maplibre-spike-filters button.active{background:#0b6668;color:#fff}.maplibre-spike-map-note{position:absolute;z-index:12;left:28px;bottom:102px;display:grid;gap:2px;padding:9px 12px;border-radius:13px;background:rgb(255 255 255/.9);box-shadow:0 8px 22px rgb(30 46 47/.12);font-size:8px}.maplibre-spike-map-note strong{font-size:9px}.maplibre-spike-map-note span{color:#6f7773}
        .maplibre-spike-outro{grid-column:1/3;grid-row:2;display:flex;align-items:center;justify-content:space-between;padding:12px 24px 15px;border-top:1px solid #ded6c9}.maplibre-spike-outro div{display:grid;gap:2px}.maplibre-spike-outro strong{font-size:17px}.maplibre-spike-outro span{font-size:9px;color:#767972}.maplibre-spike-outro em{font-family:Georgia,serif;font-size:13px;color:#777067;transform:rotate(-4deg)}
        @media(max-width:1023px){.maplibre-spike-shell{display:block;height:calc(100svh - 58px);padding:0;background:#eef2f2}.maplibre-spike-map{height:100%;border-radius:0;box-shadow:none}.maplibre-spike-map-chrome{left:12px;right:12px;top:12px;grid-template-columns:auto 1fr auto;gap:6px}.maplibre-spike-location{max-width:138px}.maplibre-spike-search{position:absolute;left:0;right:0;top:50px}.maplibre-spike-filters{left:12px;right:12px;top:112px;overflow-x:auto}.maplibre-spike-map-note{left:12px;bottom:275px}.maplibre-spike-poi-label span{max-width:180px;padding:6px 9px;font-size:8px}.maplibre-spike-neighborhood-label{font-size:11px;padding:6px 9px}.maplibre-spike-outro{display:none}}
      `}</style>
    </section>
  );
}
