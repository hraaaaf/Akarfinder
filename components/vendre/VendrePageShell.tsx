...
export async function VendrePageShell() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section data-vendre-type-entry="p1-a1" className="border-b border-border/15 bg-surface py-10 sm:py-14">
        <Container>
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div>
              <p className="text-[11px] font-extrabold uppercase tracking-[0.2em] text-primary">Vendre avec AkarFinder</p>
              <h1 className="mt-3 text-[2.45rem] font-extrabold leading-[1.03] tracking-[-0.05em] sm:text-[3.8rem]">Commençons par le type de bien</h1>
              <p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground">Choisissez le type de bien : le dossier s’adaptera ensuite, sans vous imposer de champs inutiles.</p>
      ...
      <section className="relative overflow-hidden border-b border-border/15 bg-background pb-12 pt-10 sm:pb-16 sm:pt-16">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_78%_18%,rgba(59,130,246,0.12),transparent_42%)]" />
        <Container className="relative">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-[11px] font-extrabold uppercase tracking-[0.22em] text-primary">Votre parcours</p>
            <h2 className="mt-4 text-3xl font-extrabold tracking-[-0.04em] sm:text-[3rem]">Un seul dossier clair pour avancer avec votre bien</h2>
            <p className="mx-auto mt-5 max-w-2xl text-[15px] leading-7 text-muted-foreground">Publier, estimer ou être accompagné partent des mêmes informations essentielles. Vous avancez à votre rythme, sans jargon ni publication automatique.</p>
          </div>
          <div data-vendre-paths="p1-a1" className="mx-auto mt-9 grid max-w-6xl gap-4 lg:grid-cols-3">
      ...
      <section className="border-y border-border/15 bg-surface py-14 sm:py-18">
        <Container>
