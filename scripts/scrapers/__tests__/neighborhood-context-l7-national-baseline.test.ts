...
describe("Neighborhood Context L7 — national baseline", () => {
  ...
  it("preserves fail-closed provenance and freshness for every published anchor", () => {
    const baseline = buildNeighborhoodContextNationalBaseline(NOW);
    ...
    assert.equal(anchors.every((anchor) => Boolean(anchor.source_id && anchor.attribution && anchor.observed_at)), true);
