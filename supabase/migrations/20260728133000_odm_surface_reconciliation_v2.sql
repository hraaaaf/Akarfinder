...
with matches as (
  ...
  from regexp_matches(
    coalesce(p_text,''),
    '((?:surface|superficie|terrain|parcelle|lot|terrasse|jardin|mezzanine|local[[:space:]]+commercial|commerce|magasin|appartement|villa|maison|riad|bureau|studio)?[^0-9]{0,32}([0-9]{1,6}(?:[.,][0-9]+)?)[[:space:]]*(?:m2|m²))',
    'gi'
  ) as m
), classified as (
  select
  ...
  select
    *,
    ...
    case
      when surface_type='unknown_surface_m2' then 0.45
      when surface_type='advertised_surface_m2' then 0.78
      else 0.94
    end as confidence
  from classified
)
select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'parser_version','odm_surface_parser_v2','confidence',confidence,
...
with candidates as (
  select
    a.*,
    p.resolved_registry_source_domain,p.policy_match_kind,p.policy_resolution_status,
  ...
  select p.*,
    case
      when jsonb_array_length(title_specific)>0 then title_specific
      when jsonb_array_length(snippet_specific)>0 then snippet_specific
      when jsonb_array_length(title_publishable)>0 then title_publishable
      else snippet_publishable
    end as selected_surface_candidates_v1,
    case
      when jsonb_array_length(title_specific)>0 then 'title_specific'
      when jsonb_array_length(snippet_specific)>0 then 'snippet_specific'
      when jsonb_array_length(title_publishable)>0 then 'title_advertised'
      when jsonb_array_length(snippet_publishable)>0 then 'snippet_advertised'
    end as selected_surface_evidence_source
  from pools p
)
select s.*,
  case
...
select jsonb_build_object(
 'audit_version','odm_surface_freshness_source_v2',
 'sample_size',count(*),
