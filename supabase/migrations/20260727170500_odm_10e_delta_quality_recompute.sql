-- ODM-10E correction: the ODM-10D row trigger already recomputes quality.
-- Avoid an unnecessary corpus-wide recomputation and its timeout.
...
as $$
declare
  v_before_cplus integer; v_after_cplus integer; v_evaluated integer;
  v_enriched integer; v_city integer; v_type integer; v_intent integer;
begin
  select count(*)::integer into v_before_cplus from public.thin_index_search_documents where vertical_classification='real_estate_likely' and quality_tier in ('A','B','C');
  select count(*)::integer into v_evaluated from public.thin_index_search_documents where vertical_classification='real_estate_likely' and quality_tier='D';
  with candidates as (
    select seed_id,
    ...
    from public.thin_index_search_documents where vertical_classification='real_estate_likely' and quality_tier='D'
  ), updated as (
    ...
    set normalized_city=coalesce(d.normalized_city,c.new_city),
        ...
        normalization_evidence=coalesce(d.normalization_evidence,'{}'::jsonb)||jsonb_strip_nulls(jsonb_build_object('odm_10e_run_key',p_run_key,'city_from_url',c.new_city,'property_type_from_url',c.new_type,'intent_from_url',c.new_intent,'method','canonical_url_token_match')),
        updated_at=now()
    from candidates c where d.seed_id=c.seed_id and (c.new_city is not null or c.new_type is not null or c.new_intent is not null)
    returning c.new_city,c.new_type,c.new_intent
  )
  select count(*)::integer,count(*) filter(where new_city is not null)::integer,count(*) filter(where new_type is not null)::integer,count(*) filter(where new_intent is not null)::integer into v_enriched,v_city,v_type,v_intent from updated;
  select count(*)::integer into v_after_cplus from public.thin_index_search_documents where vertical_classification='real_estate_likely' and quality_tier in ('A','B','C');
