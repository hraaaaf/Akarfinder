...
const localSources = [
  "/__qa/souissi-signature.jpg",
  "/__qa/souissi-immobilier.jpg",
  "/__qa/souissi-lifestyle.jpg",
];
  ...
  return page.evaluate(() => {
    ...
    return {
      cards: cards.map((card) => {
        ...
        const descriptors = text(card.querySelector("[data-neighborhood-template-a-descriptors]"));
        ...
        return {
          ...
          descriptorDisplay: card.querySelector("[data-neighborhood-template-a-descriptors]")
            ? getComputedStyle(card.querySelector("[data-neighborhood-template-a-descriptors]")).display
            : null,
...
try {
  for (const viewport of viewports) {
    ...
    try {
      ...
      const decodedSources = await page.evaluate(async (paths) => {
      ...
      if (decodedSources.some((source) => source.width <= 0 || source.height <= 0)) {
        throw new Error(`${viewport.name}: one or more ingested source images failed to decode`);
      }
      ...
      for (const card of metrics.cards) {
        ...
        if (viewport.width >= 640 && card.descriptorDisplay === "none") throw new Error(`${viewport.name}/${card.role}: descriptors hidden on desktop`);
