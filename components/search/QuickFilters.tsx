...
export function QuickFilters({ filters, cities, propertyTypes, onChange, onReset }: QuickFiltersProps) {
  ...
  const fieldClass =
    "min-h-11 w-full rounded-xl border border-white/12 bg-white/[0.06] px-3.5 text-[13.5px] font-semibold text-white outline-none transition placeholder:text-white/35 hover:border-bronze-400/50 focus:border-bronze-400/70 focus:ring-2 focus:ring-bronze-400/20 lg:w-auto lg:rounded-full";
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.05] p-2.5 shadow-[0_18px_50px_rgba(2,10,24,0.35)] backdrop-blur-md sm:p-3.5">
      <div className="grid gap-2.5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <label className="relative block" htmlFor="marketplace-search">
          ...
          <input
            ...
            className="h-12 w-full rounded-2xl border border-white/12 bg-white/[0.06] pl-11 pr-4 text-[15px] font-semibold text-white outline-none transition placeholder:text-white/35 focus:border-bronze-400/70 focus:ring-4 focus:ring-bronze-400/15 sm:h-14"
          />
        </label>

        <div className="grid grid-cols-3 rounded-2xl border border-white/10 bg-[#071B33]/70 p-1 text-white lg:min-w-[320px]">
            ...
            <button
              ...
              className={
                filters.transactionType === tab.value
                  ...
                  : "rounded-xl px-3 py-2.5 text-[13px] font-bold text-white/60 transition hover:text-white"
              }
            >
              {tab.label}
            </button>
      ...
      <div className="mt-2.5 flex items-center gap-2 lg:hidden">
        <button
          ...
          className="inline-flex flex-1 items-center justify-center gap-2 rounded-full border border-white/12 bg-white/[0.06] px-4 py-2.5 text-[13.5px] font-extrabold text-white"
        >
        ...
        {activeCount > 0 ? (
          <button type="button" onClick={onReset} className="rounded-full px-3 py-2.5 text-[13px] font-bold text-white/50 transition hover:text-white">
            Effacer
          </button>
        ) : null}
      </div>

      <div className={`${showFilters ? "grid" : "hidden"} mt-2.5 gap-2 border-t border-white/8 pt-3 sm:grid-cols-2 lg:mt-3 lg:flex lg:flex-wrap lg:items-center lg:gap-2 lg:border-t lg:pt-3`}>
        ...
        <div className="flex min-h-11 flex-col justify-center gap-1 rounded-xl border border-white/12 bg-white/[0.06] px-3.5 py-2 sm:col-span-2 lg:min-w-[200px] lg:rounded-full">
          <div className="flex items-center justify-between">
            <span className="text-[12px] font-bold text-white/80">Score min. fiabilité</span>
        ...
        <button type="button" onClick={() => onChange({ ...filters, mreOnly: !filters.mreOnly })} aria-pressed={filters.mreOnly}
          className={filters.mreOnly ? "min-h-11 rounded-xl bg-bronze-500/20 px-4 text-[13px] font-extrabold text-bronze-300 ring-1 ring-bronze-500/40 lg:rounded-full" : "min-h-11 rounded-xl border border-white/12 bg-white/[0.06] px-4 text-[13px] font-bold text-white/75 transition hover:border-bronze-400/50 lg:rounded-full"}>
          MRE-friendly
        </button>
        ...
        <button type="button" onClick={() => onChange({ ...filters, packageScore: filters.packageScore === "bon" ? "all" : "bon" })} aria-pressed={filters.packageScore === "bon"}
          className={filters.packageScore === "bon" ? "min-h-11 rounded-xl bg-emerald-500/20 px-4 text-[13px] font-extrabold text-emerald-300 ring-1 ring-emerald-500/40 lg:rounded-full" : "min-h-11 rounded-xl border border-white/12 bg-white/[0.06] px-4 text-[13px] font-bold text-white/75 transition hover:border-bronze-400/50 lg:rounded-full"}>
          Bon package
        </button>

        <button type="button" onClick={onReset} className="hidden min-h-11 rounded-full px-3 text-[13px] font-bold text-white/40 transition hover:bg-white/5 hover:text-white/80 lg:ml-auto lg:block">
          Réinitialiser
        </button>
      </div>
    </section>
