...
try {
  ...
  for (const vp of viewports) {
    ...
    results.push({ viewport: vp, httpStatus: response?.status() ?? null, readyState, canvasCount, shell, screenshot: file });
    await context.close();
  }
  const summary = {
  ...
  if (results.some((r) => !r.httpStatus || r.httpStatus >= 400 || r.readyState !== 'true' || r.canvasCount < 1)) process.exitCode = 2;
} finally {
  await browser?.close();
  server.kill('SIGTERM');
}
