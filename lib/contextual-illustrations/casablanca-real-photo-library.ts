...
export type ResolvedCasablancaRealPhotoAsset = CasablancaRealPhotoAsset & { contextScope: "district" };

type CertifiedNeighborhoodVisual = {
  id: string;
  source: { fileName: string; asset: string; sourcePage: string; sourceName: string };
};

function certifiedPool(district: CasablancaNeighborhood, visuals: readonly CertifiedNeighborhoodVisual[]): readonly CasablancaRealPhotoAsset[] {
...
export const CASABLANCA_REAL_PHOTO_LIBRARY: Readonly<Record<CasablancaNeighborhood, readonly CasablancaRealPhotoAsset[]>> = {
  "Maârif": certifiedPool("Maârif", MAARIF_NEIGHBORHOOD_VISUALS),
};

const DISTRICT_ALIASES: Readonly<Record<string, CasablancaNeighborhood>> = { maarif: "Maârif" };
...
export function normalizeCasablancaNeighborhood(value?: string | null): CasablancaNeighborhood | null {
...
function selectHighestRandomWeight(candidates: readonly CasablancaRealPhotoAsset[], stableKey: string): CasablancaRealPhotoAsset | null {
...
export function resolveCasablancaRealPhoto(input: { stableKey: string; city?: string | null; district?: string | null }): ResolvedCasablancaRealPhotoAsset | null {
  ...
  const selected = selectHighestRandomWeight(CASABLANCA_REAL_PHOTO_LIBRARY[district], input.stableKey);
