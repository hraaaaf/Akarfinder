-- COMMONCRAWL-RECENT-CONFIRMATION-V1.1
-- Fix search-filter compatibility by storing canonical property type / intent values.
-- The CDX/policy/content-reuse safety contract is unchanged.
...
begin
  ...
  set
    ...
    metadata = jsonb_set(
      ...
      jsonb_build_object(
        ...
        'content_reuse',false,
        'activation_version','odm_commoncrawl_recent_confirmation_v1_1'
      ),
      true
    ),
    updated_at = now()
  ...
  set
    ...
    vertical_classification_version = 'odm_commoncrawl_recent_confirmation_v1_1',
    ...
    normalization_version = 'odm_commoncrawl_recent_confirmation_v1_1',
    normalization_evidence = coalesce(d.normalization_evidence,'{}'::jsonb)
      || jsonb_build_object(
        'commoncrawl_recent_confirmation',
        jsonb_build_object('method','certified_url_pattern','batch_id',p_batch_id,'canonical_dimensions',true)
      ),
    ...
    document_kind_version = 'odm_commoncrawl_recent_confirmation_v1_1',
    updated_at = now()
  from public.odm_commoncrawl_recent_confirmation_snapshot_v1 x
  where x.batch_id = p_batch_id
    and x.batch_state = 'prepared'
  ...
  where x.batch_id = p_batch_id
    and (
      ...
      or d.normalized_city is distinct from public.odm04_normalize_city(public.odm03_recover_city(d.canonical_url))
