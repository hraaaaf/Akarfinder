...
const STATUS_COPY: Record<SellerReviewStatus, { title: string; detail: string }> = {
  ...
  approved: { title: "Dossier validé", detail: "Le dossier est validé, mais il n’est pas encore publié. La mise en ligne restera une étape séparée." },
};
    ...
    try {
      const response = await fetch(`/api/seller-drafts/${encodeURIComponent(draftId)}/review`, {
        ...
        headers: {
          ...
          "x-draft-upload-token": uploadToken,
        },
      ...
      {status === "needs_changes" ? (
        <div className="mt-5">
          ...
          <label className="mt-5 block text-sm font-extrabold">
            Ce que vous avez corrigé <span className="font-normal text-muted-foreground">(facultatif)</span>
          ...
          <button type="button" onClick={() => void resubmit()} disabled={busy} className={`${ui.primaryAction} mt-4 w-full sm:w-auto`}>
            <Send size={16} /> J’ai corrigé mon dossier
          </button>
        </div>
      ) : null}
