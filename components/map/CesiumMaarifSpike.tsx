...
function outerRings(): Array<Array<[number, number]>> {
  const geometry = maarifGeometry?.geometry;
  if (!geometry) return [];
  if (geometry.type === "Polygon") return geometry.coordinates[0] ? [Array.from(geometry.coordinates[0])] : [];
  return geometry.coordinates.flatMap((polygon) => polygon[0] ? [Array.from(polygon[0])] : []);
}
...
export function CesiumMaarifSpike() {
  ...
  useEffect(() => {
    ...
    void loadExternalAsset().then(async () => {
        ...
        rings.forEach((ring) => {
          ...
          viewer.entities.add({
            polygon: {
              ...
              material: Cesium.Color.fromCssColorString("#0b6668").withAlpha(0.10),
              outline: false,
            },
            polyline: {
      ...
      void Cesium.createOsmBuildingsAsync()
        ...
        .catch(() => { if (!disposed) setBuildings("unavailable"); });
  ...
  useEffect(() => {
    ...
    const Cesium = window.Cesium;
    if (!viewer || !Cesium || !context || contextState !== "ready") return;
  ...
  const categories = context?.categories.filter((category) => CATEGORY_META[category]) ?? [];
  const visibleAnchors = context?.anchors.filter(
    (anchor) => activeCategory === "all" || anchor.category === activeCategory,
  ) ?? [];
  ...
  return (
    ...
    >
      ...
      <div className="cesium-spike-map-note">
        ...
        <span>{contextState === "ready" ? `${visibleAnchors.length} repère${visibleAnchors.length > 1 ? "s" : ""} sourcé${visibleAnchors.length > 1 ? "s" : ""}` : "Repères indisponibles"}</span>
      </div>

      <aside className="cesium-spike-rail">
        <p className="cesium-spike-kicker">Casablanca · Quartier</p>
        <h1>Maârif</h1>
        <p className="cesium-spike-subtitle">Explorez le quartier à partir de repères publics sourcés, affichés directement sur la carte.</p>
      ...
      <style jsx global>{`
        ...
        .cesium-spike-filters{position:absolute;z-index:11;left:190px;right:404px;top:78px;display:flex;gap:7px;overflow:hidden}.cesium-spike-filters button{flex:0 0 auto;border:0;padding:7px 11px;border-radius:999px;background:rgb(255 255 255/.94);box-shadow:0 6px 18px rgb(30 46 47/.10);font-size:8px;font-weight:800;color:#364845}.cesium-spike-filters button.active{background:#0b6668;color:white}
