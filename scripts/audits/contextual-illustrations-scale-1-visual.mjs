...
const base = {
  ...
  reliability_available: true, is_mre_friendly: false, description: "Fixture déterministe SCALE-1.", image_url: "", reliability_explanation: "Fixture CI",
...
try {
  for (const viewport of viewports) {
    ...
    await page.route("**/api/search?**", async (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ listings, total: listings.length, limit: 100, offset: 0, source: "scale-1-ci", generated_at: new Date().toISOString() }) }));
    try {
      ...
      if (!metrics.visualState[24]?.contextualAssetId?.startsWith("agadir-")) throw new Error(`${viewport.name}: Agadir predecessor drift`);
      for (const fallback of metrics.visualState.slice(25, 27)) if (fallback.contextualAssetId !== null || !fallback.generic || !fallback.text.includes("Visuel illustratif")) throw new Error(`${viewport.name}: generic fallback drift`);
