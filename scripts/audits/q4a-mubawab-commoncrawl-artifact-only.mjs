...
async function latestIndexes(){return ['CC-MAIN-2026-17','CC-MAIN-2025-51','CC-MAIN-2025-30','CC-MAIN-2025-26','CC-MAIN-2025-13','CC-MAIN-2025-08','CC-MAIN-2025-05']}
function urlVariants(raw){try{const u=new URL(raw);const path=u.pathname+u.search;return [...new Set([`https://mubawab.ma${path}`,`https://www.mubawab.ma${path}`,`http://mubawab.ma${path}`,`http://www.mubawab.ma${path}`])]}catch{return[raw]}}
async function findRecord(url,indexes){for(const variant of urlVariants(url)){for(const idx of indexes){try{const q=`https://index.commoncrawl.org/${idx}-index?url=${encodeURIComponent(variant)}&output=json&filter=status:200&filter=mime:text/html&collapse=urlkey`;const r=await fetch(q,{signal:AbortSignal.timeout(5000)});if(!r.ok)continue;const text=await r.text();const lines=text.split('\n').filter(Boolean);if(lines.length){try{return JSON.parse(lines[0])}catch{}}}catch{}}}return null}
