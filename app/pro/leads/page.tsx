...
function sourceChannelLabel(channel?: string | null): string {
  ...
  if (channel === "credit") return "Financement";
  return channel ?? "—";
}
...
function creditBadge() {
  return (
    <span className="inline-flex items-center rounded-full bg-teal-100 px-2.5 py-0.5 text-[11px] font-extrabold text-teal-800">
      Crédit
    </span>
  );
}
...
function LeadCard({ lead, adminToken }: { lead: BuyerLeadRow; adminToken: string }) {
  ...
  const isCredit = lead.source_channel === "credit";
    ...
    <div className="rounded-[1.3rem] border border-[#eadfca] bg-white p-5 shadow-[0_2px_10px_rgba(7,27,51,0.06)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          {isCredit ? creditBadge() : isPromoter ? promoterBadge() : isSeller ? sellerBadge() : leadTypeBadge(leadType)}
...
export default async function LeadsPage({ searchParams }: Props) {
  ...
  const credits = normalizedLeads.filter((lead) => lead.source_channel === "credit");
      ...
      <Container className="py-10">
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-[13.5px] text-gray-500">
              {total} lead{total !== 1 ? "s" : ""} total · {sellers.length} vendeur{sellers.length !== 1 ? "s" : ""} · {promoters.length} promoteur{promoters.length !== 1 ? "s" : ""} · {credits.length} crédit · {visitRequests.length} demande{visitRequests.length !== 1 ? "s" : ""} de visite · {chaud.length} chaud{chaud.length !== 1 ? "s" : ""}
            </p>
          </div>
        ...
        <div className="mt-10 rounded-xl border border-amber-200 bg-amber-50 p-5">
          ...
          <p className="mt-3 text-[12px] text-amber-700/90">
            Dossiers acheteurs : {buyerProfiles.length} · Vendeurs : {sellers.length} · Promoteurs : {promoters.length} · Crédit : {credits.length} · Demandes de visite : {visitRequests.length}
          </p>
        </div>
      </Container>
    </div>
