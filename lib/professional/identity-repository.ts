...
export async function listCanonicalProfessionalContextsForUser(userId: string): Promise<ProfessionalMembershipContext[]> {
  ...
  const organizationIds = [...new Set(typedMemberships.map((membership) => membership.organization_id))];

  const [{ data: organizations, error: organizationError }, { data: owners, error: ownerError }] = await Promise.all([
    supabase.from("professional_organizations").select("*").in("id", organizationIds),
    supabase.from("professional_memberships").select("organization_id").in("organization_id", organizationIds).eq("role", "owner").eq("status", "active"),
  ]);
  if (organizationError) throw new Error(`[professional-identity] organizations: ${organizationError.message}`);
  if (ownerError) throw new Error(`[professional-identity] owners: ${ownerError.message}`);

  const organizationsById = new Map((organizations ?? []).map((row) => { const organization = asOrganization(row); return [organization.id, organization] as const; }));
  const organizationsWithActiveOwner = new Set((owners ?? []).map((row) => row.organization_id as string));
...
export async function convertProfessionalActivationToOrganization(input: ConvertProfessionalActivationInput): Promise<ConvertProfessionalActivationResult> {
  ...
  if (error) throw new Error(`[professional-identity] conversion: ${error.message}`);
  const row = Array.isArray(data) ? data[0] : data;
  if (!row) throw new Error("PROFESSIONAL_ACTIVATION_CONVERSION_EMPTY");
  return { activation_request_id: row.activation_request_id as string, organization_id: row.organization_id as string, membership_id: row.membership_id as string, converted_at: row.converted_at as string };
}
