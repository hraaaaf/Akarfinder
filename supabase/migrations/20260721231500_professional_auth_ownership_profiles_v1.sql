...
-- Direct Data API access is read-minimal. Mutations are performed through authenticated
-- server endpoints after membership/permission checks. service_role remains server-only.
...
using (
  (public_visibility = 'public' and validation_status = 'validated')
  or id in (
...
-- Member administration goes through server endpoints with explicit members.manage checks.
...
create policy professional_projects_public_or_member_select
on public.professional_projects for select
to anon, authenticated
using (
  (public_visibility = 'public' and status = 'published')
