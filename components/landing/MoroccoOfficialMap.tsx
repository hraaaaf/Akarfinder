const cities = [
  { name: "Tanger",     href: "/search?city=Tanger",     cx: 1325, cy: 155  },
  { name: "Fès",        href: "/search?city=F%C3%A8s",   cx: 1405, cy: 352  },
  { name: "Rabat",      href: "/search?city=Rabat",      cx: 1219, cy: 353  },
  { name: "Casablanca", href: "/search?city=Casablanca", cx: 1145, cy: 402  },
  { name: "Marrakech",  href: "/search?city=Marrakech",  cx: 1103, cy: 625  },
  { name: "Agadir",     href: "/search?city=Agadir",     cx:  942, cy: 763  },
];

export function MoroccoOfficialMap({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 2000 2000"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-label="Carte officielle du Maroc avec repères de villes"
      role="img"
      style={{
        filter:
          "drop-shadow(0 0 32px rgba(194,163,104,0.45)) drop-shadow(0 0 10px rgba(194,163,104,0.25))",
      }}
    >
      {/* Territoire officiel */}
      <path
        ...
        d="M1375.2,136.2h-18.4l-13.3,8.2h-14.8l-7.1,2.6h-10.7l-11.2,29.1l-5.1,13.8l-13.3,40.9l-7.1,28.6l-20.4,37.3
	...
	C1371.4,142.1,1377.8,142.6,1375.2,136.2z"
      ...
      {/* Pins villes — coordonnées géographiques projetées sur le SVG */}
      {cities.map(({ name, href, cx, cy }) => (
        <a key={name} href={href} aria-label={`Explorer les annonces à ${name}`}>
          <circle cx={cx} cy={cy} r="44" fill="#60a5fa" opacity="0.15" />
          <circle cx={cx} cy={cy} r="16" fill="#ffb547" stroke="white" strokeWidth="4" />
          <rect
            x={cx + 22}
            y={cy - 28}
            width={name.length * 16 + 28}
            height="36"
            rx="18"
            fill="#071a34"
            fillOpacity="0.88"
