-- ODM SEARCH ASSEMBLY SINGLE PASS FIX
-- Removes the second Ranking V2 execution from Search Assembly.
...
comment on function public.search_odm_assembled_shadow_v1(text,text,text,text,text,numeric,numeric,numeric,numeric,boolean,boolean,boolean,integer) is 'Single-pass Shadow Search Assembly: Ranking V2 executes once; controls and explanations are derived from that materialized result.';