...
async function main(){
  ...
  const {data,error}=await supabase.from("source_policy_registry").select("source_domain,authorization_status,machine_gate,ingestion_gate,display_gate,policy_expires_at").order("source_domain");
