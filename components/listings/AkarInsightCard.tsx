    ...
    >
      <div className="flex items-start justify-between gap-5">
        <div className="min-w-0">
          ...
          <h2 id="akar-insight-heading" className="mt-1 text-[1.3rem] font-extrabold tracking-[-0.035em] text-deepblue sm:text-[1.45rem]">
            Lecture du dossier
          </h2>
          {model.scoreLabel ? (
            <p data-akar-score-label className="mt-2 text-[13.5px] font-semibold leading-6 text-slate-700">
              {model.scoreLabel}
            </p>
          ) : null}
          {model.coverageLabel ? (
            <p data-akar-coverage className="mt-1 text-[11.5px] font-semibold text-slate-500">
              {model.coverageLabel}
            </p>
          ) : null}
        </div>
        ...
        {model.score != null ? (
          <div data-akar-score className="shrink-0 border-l border-[#b9d6f2] pl-4 text-right sm:pl-5">
            ...
            <p className="mt-1 text-[1.9rem] font-black leading-none tracking-[-0.05em] text-deepblue sm:text-[2.2rem]">
              {model.score}<span className="text-[0.9rem] font-extrabold text-slate-400">/100</span>
            </p>
          </div>
        ) : null}
      </div>
          ...
          {model.items.map((item) => {
            ...
            return (
              <div key={item.key} data-akar-insight-item={item.key} className="grid grid-cols-[22px_92px_minmax(0,1fr)] gap-2 py-3 sm:grid-cols-[22px_110px_minmax(0,1fr)]">
                ...
                <dt className={`text-[10px] font-extrabold uppercase tracking-[0.08em] ${warning ? "text-amber-800" : "text-slate-500"}`}>
                  {item.label}
                </dt>
                <dd className={`min-w-0 break-words text-[13px] font-semibold leading-5 ${warning ? "text-amber-900" : "text-slate-700"}`}>
                  {item.value}
                </dd>
              </div>
            );
