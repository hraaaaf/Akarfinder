...
filtered as (
  select
    (
      substr(md5(n.canonical_url),1,8) || '-' || substr(md5(n.canonical_url),9,4) || '-' ||
      substr(md5(n.canonical_url),13,4) || '-' || substr(md5(n.canonical_url),17,4) || '-' ||
      substr(md5(n.canonical_url),21,12)
    )::uuid as representation_id,
    n.*,
    case
      when n.title is not null and n.city is not null and n.price_mad is not null and n.surface_m2 is not null then 0
      when n.city is not null and (n.price_mad is not null or n.surface_m2 is not null) then 1
      else 2
    end::smallint as computed_lane_weight,
    (
      coalesce(n.quality_score,0)::real + coalesce(n.reliability_score,0)::real * 0.5
      + case when n.title is not null then 8 else 0 end
      + case when n.city is not null then 6 else 0 end
      + case when n.district is not null then 4 else 0 end
      + case when n.price_mad is not null then 5 else 0 end
      + case when n.surface_m2 is not null then 5 else 0 end
    )::real as computed_ranking_score
  from normalized n
  where (nullif(btrim(p_city),'') is null or lower(public.unaccent(n.city)) = lower(public.unaccent(btrim(p_city))))
    ...
    and (
      ...
      or not exists (
        select 1
        from unnest(regexp_split_to_array(lower(public.unaccent(btrim(p_query))), '[^a-z0-9]+')) as q(tok)
...
page as (
  ...
  order by computed_lane_weight asc, computed_ranking_score desc, updated_at desc nulls last, representation_id desc
...
select
  ...
  p.canonical_url,
  p.source_domain,
  p.canonical_source_system as seed_provider,
  p.freshness_status,
  p.title,
  p.snippet,
  p.city as normalized_city,
  p.property_type as normalized_property_type,
  p.transaction_type as normalized_intent,
  p.price_mad as normalized_price_mad,
  p.surface_m2 as normalized_surface_m2,
  p.price_per_m2_mad,
  null::text as quality_tier,
  case when p.quality_score is null then null else least(greatest(round(p.quality_score)::integer,0),100)::smallint end as quality_score,
  coalesce(p.display_eligibility,'eligible_secondary') as display_eligibility,
  'canonical_union_v1'::text as display_eligibility_reason,
  p.computed_ranking_score as ranking_quality_boost,
  p.updated_at,
  p.computed_lane_weight as lane_weight,
  p.computed_ranking_score as ranking_score,
  p.full_total_count as total_count
from page p;
$function$;
