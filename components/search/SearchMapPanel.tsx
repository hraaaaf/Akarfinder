      ...
      <div className="relative min-h-[480px] overflow-hidden lg:min-h-[640px]">
        ...
        {activeCoord && !activeHasCertifiedExactCoordinates ? (
          <span
            ...
            style={{ left: `${activeCoord.x}%`, top: `${activeCoord.y}%`, width: 120, height: 120, background: "radial-gradient(circle, rgba(37,99,235,0.25), transparent 70%)" }}
          />
        ) : null}
        ...
        {exactPropertyPoints.map((point) => {
          ...
          return (
            <button
              key={point.canonicalPropertyId}
              type="button"
              onMouseEnter={() => hoverListing(point.listing, "map")}
              onMouseLeave={clearHover}
              onFocus={() => hoverListing(point.listing, "map")}
              onBlur={clearHover}
              onClick={() => selectListing(point.listing, "map")}
              aria-label={`Sélectionner ${point.listing.title}, position exacte certifiée`}
              aria-pressed={active && selection.interaction === "selected"}
              className="group absolute z-20 -translate-x-1/2 -translate-y-1/2 focus:outline-none"
              style={{ left: `${5 + point.x * 0.9}%`, top: `${5 + point.y * 0.9}%` }}
            >
              ...
              <span className={`relative grid h-5 w-5 place-items-center rounded-full border-2 border-white shadow-[0_3px_10px_rgba(6,78,59,0.35)] transition group-hover:scale-125 ${active ? "scale-125 bg-bronze-500" : "bg-emerald-600"}`}>
                <LocateFixed size={10} className="text-white" aria-hidden="true" />
              </span>
          ...
          <div className="absolute bottom-16 left-3 right-3 z-30 rounded-2xl border border-blue-200 bg-white/95 p-4 shadow-[0_18px_40px_rgba(15,35,65,0.18)] backdrop-blur sm:left-4 sm:right-4">
            ...
            <p className="mt-2 text-[10px] leading-4 text-slate-500">
              {activeHasCertifiedExactCoordinates
                ? "Coordonnées exactes certifiées par la provenance géographique de cette fiche. La projection sur l’Atlas reste indicative."
        ...
        <div className="absolute bottom-3 left-3 right-3 z-10 rounded-xl border border-[#e4e9f2] bg-white/95 px-4 py-2.5 backdrop-blur sm:bottom-4 sm:left-4 sm:right-4 sm:rounded-2xl sm:px-5 sm:py-3">
          <p className="text-[11px] leading-4 text-slate-500">
            Cliquez un marqueur exact pour ouvrir l’aperçu, ou une ville pour filtrer · la carte ne modifie ni le classement ni l’éligibilité.
          </p>
        </div>
      </div>
    </aside>
