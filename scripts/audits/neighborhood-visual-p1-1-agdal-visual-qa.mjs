...
const outputDir = `data/audits/neighborhood-visual-p1-1-agdal/${variant}`;
const viewports = [
...
const expectedIds = ["rabat-agdal-signature-v1", "rabat-agdal-immobilier-v2", "rabat-agdal-lifestyle-v1"];
const expectedSourceDimensions = [
  { path: "/__qa/agdal-signature.jpg", width: 4160, height: 2340 },
  { path: "/__qa/agdal-immobilier.jpg", width: 320, height: 180 },
  { path: "/__qa/agdal-lifestyle.jpg", width: 4080, height: 3060 },
];
...
async function readMetrics(page) {
  return page.evaluate(({ expectedRoles }) => {
    ...
    const cards = [...document.querySelectorAll("[data-agdal-qa-card]")];
    return {
      cards: cards.map((card) => {
        ...
        return {
          ...
          mobileDetailsVisible: isVisible(mobileDetails),
          desktopDetailsVisible: isVisible(desktopDetails),
          disclosure: text(card.querySelector("[data-neighborhood-template-a-disclosure]")),
          credit: text(card.querySelector("[data-agdal-qa-credit]")),
    ...
    try {
      ...
      for (const expected of expectedSourceDimensions) {
        ...
        if (!decoded || decoded.width !== expected.width || decoded.height !== expected.height) {
          throw new Error(`${viewport.name}: source dimension drift for ${expected.path}`);
        }
      }

      const metrics = await readMetrics(page);
      if (metrics.cards.length !== 3) throw new Error(`${viewport.name}: expected 3 Agdal cards, got ${metrics.cards.length}`);
      ...
      if (JSON.stringify(roles) !== JSON.stringify(expectedRoles)) throw new Error(`${viewport.name}: role drift ${JSON.stringify(roles)}`);
      if (JSON.stringify(ids) !== JSON.stringify(expectedIds)) throw new Error(`${viewport.name}: id drift ${JSON.stringify(ids)}`);
      ...
      for (const card of metrics.cards) {
        ...
        if (card.title !== "AGDAL" || card.city !== "Rabat") throw new Error(`${viewport.name}/${card.role}: identity drift ${card.title}/${card.city}`);
        ...
        if (card.sourceKind === "project_supplied") {
          if (!card.credit.includes("AkarFinder project-supplied source") || !card.credit.includes("Autorisation directe")) {
            throw new Error(`${viewport.name}/${card.role}: project-supplied credit missing`);
          }
          if (/Wikimedia Commons|CC BY-SA/i.test(card.credit)) throw new Error(`${viewport.name}/${card.role}: false Commons/CC credit`);
        } else if (!card.credit.includes("Wikimedia Commons") || !card.credit.includes("CC BY-SA 4.0")) {
          throw new Error(`${viewport.name}/${card.role}: Commons attribution/license missing`);
        }
