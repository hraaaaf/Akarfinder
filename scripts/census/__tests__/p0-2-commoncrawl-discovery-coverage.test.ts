...
test("summary uses operational policies as the harvest-ready denominator", () => {
  const rows = buildCommonCrawlDiscoveryCoverage(
    [
      policy("ready.ma"),
  ...
  assert.equal(summary.commoncrawl_policy_domains, 3);
  assert.equal(summary.operational_policy_domains, 2);
  assert.equal(summary.harvest_ready_domains, 1);
  assert.equal(summary.pattern_missing_domains, 1);
  assert.equal(summary.expired_or_blocked_domains, 1);
  assert.equal(summary.harvest_ready_ratio, 0.5);
