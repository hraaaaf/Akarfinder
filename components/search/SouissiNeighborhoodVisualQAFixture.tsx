...
function qaListing(index: number, sceneRole: (typeof VISUALS)[number]["sceneRole"]): Listing {
  return {
    id: `souissi-visual-qa-v2-${sceneRole}`,
...
export function SouissiNeighborhoodVisualQAFixture() {
  return (
    <PropertySelectionProvider>
      <section data-souissi-visual-qa-grid className="grid grid-cols-2 gap-x-2.5 gap-y-6 lg:grid-cols-4 lg:gap-x-3">
      ...
      <style jsx global>{`
        [data-souissi-qa-card] [data-card-image] > div:first-child { opacity: 0 !important; }
        ...
        [data-souissi-qa-card] [data-card-image] > span:not([data-neighborhood-photo-title]):not([data-neighborhood-photo-disclosure]) { z-index: 20; }
      `}</style>
    </PropertySelectionProvider>
  );
}
