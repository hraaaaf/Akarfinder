...
async function requestText(url: URL): Promise<string> {
  ...
  for(let attempt=1;attempt<=4;attempt++){
    try{
      const r=await fetch(url,{headers:{'user-agent':'AkarFinder-Q1A-DATA49B-sitemap-root-discovery/1.1 metadata-only'},signal:AbortSignal.timeout(30_000)});
...
async function queryDomain(index:string):Promise<string[]>{
  const base=`https://index.commoncrawl.org/${index}-index`;
  const params=new URLSearchParams({url:domain,matchType:'domain',output:'json',fl:'url',filter:'status:200',collapse:'urlkey'});
  ...
  for(let page=0;page<pages;page++){
    ...
    for(const line of text.split('\n')){
      ...
      try{const row=JSON.parse(t) as {url?:string};if(row.url && /sitemap/i.test(row.url))out.add(row.url);}catch{}
    }
  }
  return [...out];
}
...
async function main(){
  ...
  for(const index of INDEXES){
    ...
    try{for(const u of await queryDomain(index))urls.add(u);}catch(e){errors.push(`${index}: ${e instanceof Error?e.message:String(e)}`);}
    found.set(index,urls);
  }
  const all=[...new Set([...found.values()].flatMap(s=>[...s]))].sort();
  ...
  const summary={schemaVersion:'Q1A_DATA49B_SITEMAP_ROOT_DISCOVERY_V2',domain,cutoff:CUTOFF,indexes:INDEXES,readOnly:true,databaseWrites:0,productionWrites:0,sourceSiteFetches:0,sourceContentFetches:0,warcFetches:0,commonCrawlUrlIndexRequestsOnly:true,vercelDeployments:0,errors,queryComplete:errors.length===0,perIndex:Object.fromEntries([...found].map(([k,v])=>[k,[...v].sort()])),distinctSitemapLikeUrls:all.length,roots};
